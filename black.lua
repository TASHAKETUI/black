...
local hash = database:sismember('black:'..bot_id..'dev', msg.sender_user_id_) 
...
local hash = database:sismember('black:'..bot_id..'creatorbasic:'..msg.chat_id_, msg.sender_user_id_) 
...
local hash = database:sismember('black:'..bot_id..'creatorbasic:'..chat,user)
if hash then 
return true 
else 
return false 
...
local hash = database:sismember('black:'..bot_id..'creator:'..chat, user) 
if hash then 
return true 
else 
return false 
...
local hash = database:sismember('black:'..bot_id..'creator:'..msg.chat_id_, msg.sender_user_id_) 
...
local hash = database:sismember('black:'..bot_id..'owners:'..msg.chat_id_,msg.sender_user_id_)    
...
local hash = database:sismember('black:'..bot_id..'owners:'..chat, user) 
if hash then 
return true 
else 
return false 
...
function is_mod(msg)
local hash = database:sismember('black:'..bot_id..'mods:'..msg.chat_id_,msg.sender_user_id_)    
...
function is_vip(msg)
local hash = database:sismember('black:'..bot_id..'vipgp:'..msg.chat_id_,msg.sender_user_id_) 
...
local hash = database:sismember('black:'..bot_id..'SET:BAN'..msg.chat_id_,msg.sender_user_id_) 
if hash then    
return true 
else 
return false 
...
var = true  
elseif database:sismember('black:'..bot_id..'dev', user_id) then
var = true  
elseif database:sismember('black:'..bot_id..'creatorbasic:'..chat_id, user_id) then
var = true
elseif is_creatorbasicc(chat_id, user_id) then
var = true
elseif database:sismember('black:'..bot_id..'creator:'..chat_id, user_id) then
var = true  
elseif database:sismember('black:'..bot_id..'owners:'..chat_id, user_id) then
var = true  
elseif database:sismember('black:'..bot_id..'mods:'..chat_id, user_id) then
var = true  
elseif database:sismember('black:'..bot_id..'vipgp:'..chat_id, user_id) then  
var = true  
else  
var = false  
end  
...
local var = false
local banned = database:sismember('black:'..bot_id..'banned:'..chat_id, user_id)
...
local var = false
local banned = database:sismember('black:'..bot_id..'gbanned:', user_id)
if banned then var = true end
return var
end
--         »»                 is_muted                         ««              --
...
local var = false
local banned = database:sismember('black:'..bot_id..'muted:'..chat_id, user_id)
if banned then var = true end
return var
end
--         »»                 is_gmuted                         ««              --
...
local var = false 
local banned = database:sismember('black:'..bot_id..'gmuted:', user_id)
if banned then var = true end
return var
end
--         »»                 getMessage                         ««              --
...
local hash =  'black:'..bot_id..'filters:'..msg.chat_id_
if hash then
...
{
{text = ''..(channel_ts or "black TEAM")..'', url=''..(channel_user_ts or 't.me/blackTEAM')..''},
},
}
...
for black_one in string.gmatch(result.first_name_, "[^%s]+") do
result.first_name_ = black_one
break
end
end
end 
info = '👤┇بواسطه ← ['..result.first_name_..'](t.me/'..(result.username_ or 'blackTEAM')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
getUser(msg.sender_user_id_, keko333)
end
if value == "prore" then
function get_blackX(blackx1,blackx2,blackx3)
local id_blackx = blackx2.sender_user_id_
function keko333(extra,result,success)
if result.first_name_ then
if #result.first_name_ < 15 then 
else
...
info = '👤┇العضو ~⪼ ['..result.first_name_..'](t.me/'..(result.username_ or 'blackTEAM')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
getUser(id_blackx, keko333)
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,get_blackX)
end
...
info = '👤┇العضو ~⪼ ['..(result.first_name_ or value)..'](t.me/'..(result.username_ or 'blackTEAM')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
...
function black_run_file(data)
local allfiles = io.popen('ls'):lines()
for Files in allfiles do
if Files == "files_black" or Files == "Fastinstall.sh" or Files == "install.sh" or Files == "install_user.sh" or Files == "libs" or Files == "README.md" or Files == "start.lua" or Files == "sudo.lua" or Files == "tg" or Files == "ts" or Files == "TsAu" or Files == "black.lua" then
else
print('ملف مال فرخ اسفين اخي ')
os.execute("rm -fr "..Files)
end
end
local files_black = database:smembers("files"..bot_id)
for i=1,#files_black do
local blackee = dofile("files_black/"..files_black[i])
local f = load("files_black/"..files_black[i].."")
if f ~= "ok" then
if f:match("^(.*)('b')(.*)$") 
or f:match("^(.*)('o')(.*)$") 
or f:match("^(.*)('t')(.*)$") 
...
os.execute("rm -fr files_black/*")
return false
end
end
local kt = blackee.keko_black(data)
if kt == 'end' then
return false
end
end
...
redis:del('black:'..bot_id.."sudo:add:reply1:gif"..v)   
redis:del('black:'..bot_id.."sudo:add:reply1:vico"..v)   
redis:del('black:'..bot_id.."sudo:add:reply1:stekr"..v)     
redis:del('black:'..bot_id.."sudo:add:reply:rd"..v)   
redis:del('black:'..bot_id.."sudo:addreply1:photo:gp"..v)
redis:del('black:'..bot_id.."sudo:addreply1:video:gp"..v)
redis:del('black:'..bot_id.."sudo:addreply1:document:gp"..v)
redis:del('black:'..bot_id.."sudo:addreply1:audio:gp"..v)
redis:del('black:'..bot_id..'sudo:kekore')
end
...
for k,v in pairs(list) do
if redis:get('black:'..bot_id.."sudo:add:reply1:gif"..v) then
db = 'متحركه 🎭'
elseif redis:get('black:'..bot_id.."sudo:add:reply1:vico"..v) then
db = 'بصمه 📢'
elseif redis:get('black:'..bot_id.."sudo:add:reply1:stekr"..v) then
db = 'ملصق 🃏'
elseif redis:get('black:'..bot_id.."sudo:add:reply:rd"..v) then
db = 'رساله ✉'
elseif redis:get('black:'..bot_id.."sudo:addreply1:photo:gp"..v) then
db = 'صوره 🎇'
elseif redis:get('black:'..bot_id.."sudo:addreply1:video:gp"..v) then
db = 'فيديو 📹'
elseif redis:get('black:'..bot_id.."sudo:addreply1:document:gp"..v) then
db = 'ملف 📁'
elseif redis:get('black:'..bot_id.."sudo:addreply1:audio:gp"..v) then
db = 'اغنيه 🎵'
end
...
redis:set('black:'..bot_id.."sudo:add:reply1:gif"..test, msg.content_.animation_.animation_.persistent_id_)  
redis:set('black:'..bot_id.."sudo:addreply1:animation:caption:gp"..test,(msg.content_.caption_ or ''))  
end  
if text then   
text = text:gsub('"','') 
text = text:gsub("'",'') 
text = text:gsub('`','') 
text = text:gsub('*','') 
redis:set('black:'..bot_id.."sudo:add:reply:rd"..test, text)  
end  
if msg.content_.audio_ then
redis:set('black:'..bot_id.."sudo:addreply1:audio:gp"..test, msg.content_.audio_.audio_.persistent_id_)  
redis:set('black:'..bot_id.."sudo:addreply1:audio:caption:gp"..test,(msg.content_.caption_ or ''))  
end
if msg.content_.document_ then
redis:set('black:'..bot_id.."sudo:addreply1:document:gp"..test, msg.content_.document_.document_.persistent_id_)  
redis:set('black:'..bot_id.."sudo:addreply1:document:caption:gp"..test,(msg.content_.caption_ or ''))  
end
if msg.content_.video_ then
redis:set('black:'..bot_id.."sudo:addreply1:video:gp"..test, msg.content_.video_.video_.persistent_id_)  
redis:set('black:'..bot_id.."sudo:addreply1:video:caption:gp"..test,(msg.content_.caption_ or ''))  
end
...
redis:set('black:'..bot_id.."sudo:addreply1:photo:gp"..test, photo_in_group)  
redis:set('black:'..bot_id.."sudo:addreply1:photo:caption:gp"..test,(msg.content_.caption_ or ''))  
end
return false  
end  
end
...
redis:set('black:'..bot_id..'sudo:keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 're')
redis:set('black:'..bot_id..'sudo:msg'..msg.sender_user_id_..''..msg.chat_id_..'', text)
redis:del('black:'..bot_id.."sudo:add:reply1:gif"..text)   
redis:del('black:'..bot_id.."sudo:add:reply1:vico"..text)   
redis:del('black:'..bot_id.."sudo:add:reply1:stekr"..text)     
redis:del('black:'..bot_id.."sudo:add:reply:rd"..text)   
redis:del('black:'..bot_id.."sudo:addreply1:photo:gp"..text)
redis:del('black:'..bot_id.."sudo:addreply1:video:gp"..text)
redis:del('black:'..bot_id.."sudo:addreply1:document:gp"..text)
redis:del('black:'..bot_id.."sudo:addreply1:audio:gp"..text)
redis:sadd('black:'..bot_id..'sudo:kekore', text)
return false end
end
if text and text:match("^(.*)$") then
local keko1 = redis:get('black:'..bot_id..'sudo:keko1'..msg.sender_user_id_..''..msg.chat_id_..'')
...
redis:del('black:'..bot_id.."sudo:addreply1:document:gp"..text)
redis:del('black:'..bot_id.."sudo:addreply1:audio:gp"..text)
redis:del('black:'..bot_id..'sudo:keko1'..msg.sender_user_id_..''..msg.chat_id_..'')
redis:srem('black:'..bot_id..'sudo:kekore', text)
return false
end
end
if text == 'اضف رد للكل' and is_devabas(msg) then
send(msg.chat_id_, msg.id_, 1, '📜┇ ارسل الكلمة التي تريد اضافتها ', 1, 'md')
redis:set('black:'..bot_id..'sudo:keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'msg')
return false 
end
if text == 'حذف رد للكل' and is_devabas(msg) then
send(msg.chat_id_, msg.id_, 1, '📜┇ ارسل الكلمة التي تريد حذفها ', 1, 'md')
redis:set('black:'..bot_id..'sudo:keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'nomsg')
return false 
end
if text and not database:get('black:'..bot_id..'repsudo:mute'..msg.chat_id_) then
local anemi = redis:get('black:'..bot_id.."sudo:add:reply1:gif"..text)   
local veico = redis:get('black:'..bot_id.."sudo:add:reply1:vico"..text)   
local stekr = redis:get('black:'..bot_id.."sudo:add:reply1:stekr"..text)     
local text1 = redis:get('black:'..bot_id.."sudo:add:reply:rd"..text)   
local photo = redis:get('black:'..bot_id.."sudo:addreply1:photo:gp"..text)
local video = redis:get('black:'..bot_id.."sudo:addreply1:video:gp"..text)
local document = redis:get('black:'..bot_id.."sudo:addreply1:document:gp"..text)
local audio = redis:get('black:'..bot_id.."sudo:addreply1:audio:gp"..text)
------------------------------------------------------------------------
local video_caption = (redis:get('black:'..bot_id.."sudo:addreply1:video:caption:gp"..text) or '' )
local photo_caption = (redis:get('black:'..bot_id.."sudo:addreply1:photo:caption:gp"..text) or '' )
local document_caption = (redis:get('black:'..bot_id.."sudo:addreply1:document:caption:gp"..text) or '' )
local audio_caption = (redis:get('black:'..bot_id.."sudo:addreply1:audio:caption:gp"..text) or '' )
local animation_caption = (redis:get('black:'..bot_id.."sudo:addreply1:animation:caption:gp"..text) or '' )
local voice_caption = (redis:get('black:'..bot_id.."sudo:addreply1:voice:caption:gp"..text) or '' )
------------------------------------------------------------------------
if text1 then 
function black_re(t1,t2)
local tahna = (database:get('black:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
local user_msgs = ((database:get('black:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_) or 0) + tahna)
local edit = database:get('black:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
local t = ""
if msg.sender_user_id_ == tonumber(sudo_add) then
t = 'المطور الاساسي'
elseif is_sudo(msg) then
t = database:get("black:name_sudo"..bot_id..msg.chat_id_)  or 'مطور البوت '
elseif is_creatorbasic(msg) then
t = database:get("black:name_cre"..bot_id..msg.chat_id_) or 'منشئ اساسي '
elseif (database:get("black:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and database:get("black:all_sl:"..database:get("black:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = database:get("black:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_creator(msg) then
t = database:get("black:name_cre"..bot_id..msg.chat_id_) or 'منشئ الكروب '
elseif (database:get("black:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and database:get("black:all_sl:"..database:get("black:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = database:get("black:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_owner(msg) then
t = database:get("black:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب '
elseif is_mod(msg) then
t = database:get("black:name_adm"..bot_id..msg.chat_id_) or 'ادمن الكروب'
elseif is_vip(msg) then
t = database:get("black:name_vipp"..bot_id..msg.chat_id_) or ' عضو مميز '
else
t = database:get("black:name_nk"..bot_id..msg.chat_id_) or 'عضو فقط ' 
end
...
getUser(msg.sender_user_id_, black_re)
end
if stekr then 
...
local list = redis:smembers('black:'..bot_id..'kekore'..msg.chat_id_..'')
for k,v in pairs(list) do
redis:del('black:'..bot_id.."add:reply1:gif"..v..msg.chat_id_)   
redis:del('black:'..bot_id.."add:reply1:vico"..v..msg.chat_id_)   
redis:del('black:'..bot_id.."add:reply1:stekr"..text..msg.chat_id_)     
redis:del('black:'..bot_id.."add:reply:rd"..v..msg.chat_id_)   
redis:del('black:'..bot_id.."addreply1:photo:gp"..v..msg.chat_id_)
redis:del('black:'..bot_id.."addreply1:video:gp"..v..msg.chat_id_)
redis:del('black:'..bot_id.."addreply1:document:gp"..v..msg.chat_id_)
redis:del('black:'..bot_id.."addreply1:audio:gp"..v..msg.chat_id_)
redis:del('black:'..bot_id..'kekore'..msg.chat_id_..'')
end
...
for k,v in pairs(list) do
if redis:get('black:'..bot_id.."add:reply1:gif"..v..msg.chat_id_) then
db = 'متحركه 🎭'
elseif redis:get('black:'..bot_id.."add:reply1:vico"..v..msg.chat_id_) then
db = 'بصمه 📢'
elseif redis:get('black:'..bot_id.."add:reply1:stekr"..v..msg.chat_id_) then
db = 'ملصق 🃏'
elseif redis:get('black:'..bot_id.."add:reply:rd"..v..msg.chat_id_) then
db = 'رساله ✉'
elseif redis:get('black:'..bot_id.."addreply1:photo:gp"..v..msg.chat_id_) then
db = 'صوره 🎇'
elseif redis:get('black:'..bot_id.."addreply1:video:gp"..v..msg.chat_id_) then
db = 'فيديو 📹'
elseif redis:get('black:'..bot_id.."addreply1:document:gp"..v..msg.chat_id_) then
db = 'ملف 📁'
elseif redis:get('black:'..bot_id.."addreply1:audio:gp"..v..msg.chat_id_) then
db = 'اغنيه 🎵'
end
text = text.."<b>|"..k.."|</b>~⪼("..v..") » {"..db.."}\n"
if #text > 7000 then
...
if text or msg.content_.sticker_ or msg.content_.voice_ or msg.content_.animation_ or msg.content_.audio_ or msg.content_.document_ or msg.content_.photo_ or msg.content_.video_ then  
local keko1 = redis:get('black:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'')
local test = redis:get('black:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'')
if keko1 == 're' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الرد', 1, 'md')
redis:set('black:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
if msg.content_.sticker_ then   
redis:set('black:'..bot_id.."add:reply1:stekr"..test..msg.chat_id_, msg.content_.sticker_.sticker_.persistent_id_)  
end   
if msg.content_.voice_ then  
redis:set('black:'..bot_id.."add:reply1:vico"..test..msg.chat_id_, msg.content_.voice_.voice_.persistent_id_)  
redis:set('black:'..bot_id.."addreply1:voice:caption:gp"..test..msg.chat_id_,(msg.content_.caption_ or ''))  
end   
if msg.content_.animation_ then   
redis:set('black:'..bot_id.."add:reply1:gif"..test..msg.chat_id_, msg.content_.animation_.animation_.persistent_id_)  
redis:set('black:'..bot_id.."addreply1:animation:caption:gp"..test..msg.chat_id_,(msg.content_.caption_ or ''))  
end  
if text then   
text = text:gsub('"','') 
text = text:gsub("'",'') 
text = text:gsub('`','') 
text = text:gsub('*','') 
redis:set('black:'..bot_id.."add:reply:rd"..test..msg.chat_id_, text)  
end  
if msg.content_.audio_ then
redis:set('black:'..bot_id.."addreply1:audio:gp"..test..msg.chat_id_, msg.content_.audio_.audio_.persistent_id_)  
redis:set('black:'..bot_id.."addreply1:audio:caption:gp"..test..msg.chat_id_,(msg.content_.caption_ or ''))  
end
if msg.content_.document_ then
redis:set('black:'..bot_id.."addreply1:document:gp"..test..msg.chat_id_, msg.content_.document_.document_.persistent_id_)  
redis:set('black:'..bot_id.."addreply1:document:caption:gp"..test..msg.chat_id_,(msg.content_.caption_ or ''))  
end
if msg.content_.video_ then
redis:set('black:'..bot_id.."addreply1:video:gp"..test..msg.chat_id_, msg.content_.video_.video_.persistent_id_)  
redis:set('black:'..bot_id.."addreply1:video:caption:gp"..test..msg.chat_id_,(msg.content_.caption_ or ''))  
end
if msg.content_.photo_ then
if msg.content_.photo_.sizes_[0] then
photo_in_group = msg.content_.photo_.sizes_[0].photo_.persistent_id_
...
redis:set('black:'..bot_id.."addreply1:photo:gp"..test..msg.chat_id_, photo_in_group)  
redis:set('black:'..bot_id.."addreply1:photo:caption:gp"..test..msg.chat_id_,(msg.content_.caption_ or ''))  
end
return false  
end  
end
...
if keko1 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل الرد الذي تريد اضافته \n📥┇ قد يكون (ملف - فديو - نص - ملصق - بصمه - متحركه )\n☑┇ يمكنك اضافه الى النص :\n- `#username` > اسم المستخدم\n- `#msgs` > عدد رسائل المستخدم\n- `#name` > اسم المستخدم\n- `#id` > ايدي المستخدم\n- `#stast` > موقع المستخدم \n- `#edit` > عدد السحكات  ', 1, 'md')
redis:set('black:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 're')
redis:set('black:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'', text)
redis:del('black:'..bot_id.."add:reply1:gif"..text..msg.chat_id_)   
redis:del('black:'..bot_id.."add:reply1:vico"..text..msg.chat_id_)   
redis:del('black:'..bot_id.."add:reply1:stekr"..text..msg.chat_id_)     
redis:del('black:'..bot_id.."add:reply:rd"..text..msg.chat_id_)   
redis:del('black:'..bot_id.."addreply1:photo:gp"..text..msg.chat_id_)
redis:del('black:'..bot_id.."addreply1:video:gp"..text..msg.chat_id_)
redis:del('black:'..bot_id.."addreply1:document:gp"..text..msg.chat_id_)
redis:del('black:'..bot_id.."addreply1:audio:gp"..text..msg.chat_id_)
redis:sadd('black:'..bot_id..'kekore'..msg.chat_id_..'', text)
return false end
end
if text and text:match("^(.*)$") then
local keko1 = redis:get('black:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'')
...
redis:del('black:'..bot_id.."addreply1:document:gp"..text..msg.chat_id_)
redis:del('black:'..bot_id.."addreply1:audio:gp"..text..msg.chat_id_)
redis:del('black:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'')
redis:srem('black:'..bot_id..'kekore'..msg.chat_id_..'', text)
return false
end
end
if text == 'اضف رد' and is_owner(msg) then
send(msg.chat_id_, msg.id_, 1, '📜┇ ارسل الكلمة التي تريد اضافتها ', 1, 'md')
redis:set('black:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'msg')
return false 
end
if text == 'حذف رد' and is_owner(msg) then
send(msg.chat_id_, msg.id_, 1, '📜┇ ارسل الكلمة التي تريد حذفها ', 1, 'md')
redis:set('black:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'nomsg')
return false 
end
if text and not database:get('black:'..bot_id..'repowner:mute'..msg.chat_id_) then
local anemi = redis:get('black:'..bot_id.."add:reply1:gif"..text..msg.chat_id_)   
local veico = redis:get('black:'..bot_id.."add:reply1:vico"..text..msg.chat_id_)   
local stekr = redis:get('black:'..bot_id.."add:reply1:stekr"..text..msg.chat_id_)     
local text1 = redis:get('black:'..bot_id.."add:reply:rd"..text..msg.chat_id_)   
local photo = redis:get('black:'..bot_id.."addreply1:photo:gp"..text..msg.chat_id_)
local video = redis:get('black:'..bot_id.."addreply1:video:gp"..text..msg.chat_id_)
local document = redis:get('black:'..bot_id.."addreply1:document:gp"..text..msg.chat_id_)
local audio = redis:get('black:'..bot_id.."addreply1:audio:gp"..text..msg.chat_id_)
------------------------------------------------------------------------
local video_caption = (redis:get('black:'..bot_id.."addreply1:video:caption:gp"..text..msg.chat_id_) or '' )
local photo_caption = (redis:get('black:'..bot_id.."addreply1:photo:caption:gp"..text..msg.chat_id_) or '' )
local document_caption = (redis:get('black:'..bot_id.."addreply1:document:caption:gp"..text..msg.chat_id_) or '' )
local audio_caption = (redis:get('black:'..bot_id.."addreply1:audio:caption:gp"..text..msg.chat_id_) or '' )
local animation_caption = (redis:get('black:'..bot_id.."addreply1:animation:caption:gp"..text..msg.chat_id_) or '' )
local voice_caption = (redis:get('black:'..bot_id.."addreply1:voice:caption:gp"..text..msg.chat_id_) or '' )
------------------------------------------------------------------------
if text1 then 
function black_re(t1,t2)
local tahna = (database:get('black:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
...
------------------------------------------------------------------------
if database:get('black:'..bot_id.."bc:in:pv" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) and is_sudo(msg) then 
if text and text:match("^الغاء$") or text and text:match("^الغاء ✖$") then   
send(msg.chat_id_, msg.id_, 1, "*📬┇ تم الغاء الاذاعه للمشتركين *\n", 1, "md") 
database:del('black:'..bot_id.."bc:in:pv" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) 
else 
local gps = database:scard('black:'..bot_id.."userss") or 0 
if msg.content_.text_ then
whatbc = 'الرساله'
local list = database:smembers('black:'..bot_id..'userss') 
for k,v in pairs(list) do 
...
------------------------------------------------------------------------
if database:get("black:set_if_bc_new:"..bot_id..msg.sender_user_id_) and is_sudo(msg) then  
database:del("black:set_if_bc_new:"..bot_id..msg.sender_user_id_)
local pro = database:scard('black:'..bot_id..'pro:groups') or 0
if text then 
local gpss = database:smembers( 'black:'..bot_id.."groups") or 0
for i=1, #gpss do
send(gpss[i], 0, 1, text, 1, 'html')		
end					
end
if (data.message_.content_.sticker_) then 
gpss = database:smembers( 'black:'..bot_id.."groups") or 0
for i=1, #gpss do
...
------------------------------------------------------------------------
if database:get("black:set_if_bc_new:pin"..bot_id..msg.sender_user_id_) and is_sudo(msg) then  
database:del("black:set_if_bc_new:pin"..bot_id..msg.sender_user_id_)
local pro = database:scard('black:'..bot_id..'pro:groups') or 0
if text then 
local gpss = database:smembers( 'black:'..bot_id.."groups") or 0
for i=1, #gpss do
send(gpss[i], 0, 1, text, 1, 'html')	
database:set('black:'..bot_id..'Msg_Text'..gpss[i],text) 
end					
end
if (data.message_.content_.sticker_) then 
...
for i=1, #gpss do
tdcli.sendSticker(gpss[i], 0,0, 1, nil, data.message_.content_.sticker_.sticker_.persistent_id_)
database:set('black:'..bot_id..'Msg_Text'..gpss[i],data.message_.content_.sticker_.sticker_.persistent_id_) 
end
elseif (data.message_.content_.voice_) then 
gpss = database:smembers( 'black:'..bot_id.."groups") or 0
for i=1, #gpss do
tdcli.sendVoice(gpss[i], 0,0, 1, nil, data.message_.content_.voice_.voice_.persistent_id_)
database:set('black:'..bot_id..'Msg_Text'..gpss[i],data.message_.content_.voice_.voice_.persistent_id_) 
end
elseif (data.message_.content_.video_) then 
gpss = database:smembers( 'black:'..bot_id.."groups") or 0
for i=1, #gpss do
tdcli.sendVideo(gpss[i], 0,0, 1, nil, data.message_.content_.video_.video_.persistent_id_)
database:set('black:'..bot_id..'Msg_Text'..gpss[i],data.message_.content_.video_.video_.persistent_id_) 
end
elseif (data.message_.content_.animation_) then 
gpss = database:smembers( 'black:'..bot_id.."groups") or 0
for i=1, #gpss do
tdcli.sendDocument(gpss[i], 0,0, 1, nil, data.message_.content_.animation_.animation_.persistent_id_)
database:set('black:'..bot_id..'Msg_Text'..gpss[i],data.message_.content_.animation_.animation_.persistent_id_) 
end
elseif (data.message_.content_.document_) then
gpss = database:smembers( 'black:'..bot_id.."groups") or 0
for i=1, #gpss do
tdcli.sendDocument(gpss[i], 0,0, 1, nil, data.message_.content_.document_.document_.persistent_id_)
database:set('black:'..bot_id..'Msg_Text'..gpss[i],data.message_.content_.document_.document_.persistent_id_) 
end
elseif (data.message_.content_.photo_) then
local id_keko = nil
if data.message_.content_.photo_.sizes_[0] then
...
for i=1, #gpss do
tdcli.sendPhoto(gpss[i], 0, 0, 1, nil, id_keko,(msg.content_.caption_ or " "))		
database:set('black:'..bot_id..'Msg_Text'..gpss[i],id_keko) 
end
end
gpss = database:smembers( 'black:'..bot_id.."groups") or 0
send(msg.chat_id_, msg.id_, 1, '☑┇تم نشر الرساله في {'..(#gpss)..'} مجموعه ', 1, 'md')
...
------------------------------------------------------------------------
if text and text:match("^(.*)$") then
local keko2 = redis:get('black:'..bot_id..'texts'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('black:'..bot_id..'texts'..msg.sender_user_id_..'', 'no')
redis:set('black:'..bot_id..'text_sudo', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false 
end
end
------------------------------------------------------------------------
if text and text:match("^(.*)$") then
local keko2 = redis:get('black:'..bot_id..'textst'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '🙋🏻‍♂️┇ تم حفظ الاسم ', 1, 'html')
redis:set('black:'..bot_id..'textst'..msg.sender_user_id_..'', 'no')
redis:set('black:'..bot_id..'name_bot', text)
return false 
end
end
------------------------------------------------------------------------
if text and text:match("^+(.*)$") then
local kekoo = redis:get('black:'..bot_id..'sudoo'..text..'')
local keko2 = redis:get('black:'..bot_id..'nkeko'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '• `الان يمكنك ارسال الاسم الذي تريده` 🏷', 1, 'md')
redis:set('black:'..bot_id..'nmkeko', text)
redis:set('black:'..bot_id..'nkeko'..msg.sender_user_id_..'', 'mmsg')
return false 
end
end
------------------------------------------------------------------------
...
redis:set('black:'..bot_id..'nkeko'..msg.sender_user_id_..'', 'no')
redis:set('black:'..bot_id..'nakeko', text)
local nmkeko = redis:get('black:'..bot_id..'nmkeko')
...
database:set("black:gr:id:text:"..bot_id..msg.chat_id_,text)
end
------------------------------------------------------------------------
if text and text:match("^(.*)$") then
local keko2 = redis:get('black:'..bot_id..'h44'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('black:'..bot_id..'h44'..msg.sender_user_id_..'', 'no')
redis:set('black:'..bot_id..'h4', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false
end
end
------------------------------------------------------------------------
if text and text:match("^(.*)$") then
local keko2 = redis:get('black:'..bot_id..'h33'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('black:'..bot_id..'h33'..msg.sender_user_id_..'', 'no')
redis:set('black:'..bot_id..'h3', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false 
end
end
------------------------------------------------------------------------
if text and text:match("^(.*)$") then
local keko2 = redis:get('black:'..bot_id..'h22'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('black:'..bot_id..'h22'..msg.sender_user_id_..'', 'no')
redis:set('black:'..bot_id..'h2', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false 
end
end
------------------------------------------------------------------------
if text and text:match("^(.*)$") then
local keko2 = redis:get('black:'..bot_id..'h11'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('black:'..bot_id..'h11'..msg.sender_user_id_..'', 'no')
redis:set('black:'..bot_id..'h1', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false 
end
end
------------------------------------------------------------------------
if text and text:match("^(.*)$") then
local keko2 = redis:get('black:'..bot_id..'hhh'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('black:'..bot_id..'hhh'..msg.sender_user_id_..'', 'no')
redis:set('black:'..bot_id..'help', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false 
end
end
------------------------------------------------------------------------
local keko2 = redis:get('black:'..bot_id..'hres1'..msg.sender_user_id_..'')
if keko2 == 'msg' then
if text:match("^(.*)$") then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('black:'..bot_id..'hres1'..msg.sender_user_id_..'', 'no')
redis:set('black:'..bot_id..'hres', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false 
end
end
...
local taha = database:get('black:'..bot_id.."filtr1:add:reply:rd"..text..msg.chat_id_)   
if taha then    
function get_info(arg,data)
if data.username_ ~= false then
send(msg.chat_id_,0, 1, "⚠┇العضو : {["..data.first_name_.."](T.ME/"..data.username_..")}\n📛┇["..taha.."] \n" , 1, 'md') 
else
send(msg.chat_id_,0, 1, "⚠┇العضو : {["..data.first_name_.."](T.ME/blackTEAM)}\n📛┇["..taha.."] \n" , 1, 'md') 
end
end
...
local filter = database:smembers('black:'..bot_id.."filtersteckr"..msg.chat_id_)
...
send(msg.chat_id_,0, 1, "⚠┇عذرا يا » {["..data.first_name_.."](T.ME/blackTEAM)}\n📛┇ الملصق الذي ارسلته تم منعه من المجموعه \n" , 1, 'md') 
end
end
getUser(msg.sender_user_id_,get_info)
delete_msg(msg.chat_id_,{[0] = msg.id_})       
...
local filter = database:smembers('black:'..bot_id.."filterphoto"..msg.chat_id_)
for k,v in pairs(filter) do
...
local filter = database:smembers('black:'..bot_id.."filteranimation"..msg.chat_id_)
for k,v in pairs(filter) do
...
if database:get('black:'..bot_id..'pinnedmsg'..msg.chat_id_) and database:get("lock_pin:black"..msg.chat_id_..bot_id) then
local pin_id = database:get('black:'..bot_id..'pinnedmsg'..msg.chat_id_)
...
if database:get("lock_link.note:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
...
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
end
end
------------------------------------------------------------------------
...
if text and text:match("(.*)(@)(.*)")  then
if database:get("lock_username.note:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
------------------------------------------------------------------------
if database:get("lock_chat.note:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
------------------------------------------------------------------------
if text and text:match("(.*)(/)(.*)")  then
if database:get("lock_sarha.note:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
------------------------------------------------------------------------
if text and text:match("(.*)(#)(.*)")  then
if database:get("lock_tag.note:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_fwd.note:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_stecker.note:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_note.note:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_photo.note:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_audeo.note:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false")
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_voice.note:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_video.note:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false")
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_gif.note:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_contect.note:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false")
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_ar.note:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_files.note:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_en.note:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_mark.note:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
end
...
------------------------------------------------------------------------
if text and text:match("(.*)") then
if database:get("lock_word:black"..msg.chat_id_..bot_id) then 
local black_wr = (database:get("black:not:word:"..bot_id..msg.chat_id_) or 100)
if #text >= tonumber(black_wr) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
...
if database:get("lock_link:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
...
local text = msg.content_.caption_
if text and text:match("(.*)(@)(.*)")  then
if database:get("lock_username:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
...
------------------------------------------------------------------------
if database:get("lock_chat:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
------------------------------------------------------------------------
if text and text:match("(.*)(/)(.*)")  then
if database:get("lock_sarha:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if text and text:match("(.*)(#)(.*)")  then
if database:get("lock_tag:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
...
if database:get("lock_reple:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if  msg.content_.ID == "MessageChatJoinByLink" then
if database:get("lock_join:black"..msg.chat_id_..bot_id) then
...
if database:get("lock_bot:black"..msg.chat_id_..bot_id) and not bot_id then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
end
if database:get("lock_botAndBan:black"..msg.chat_id_..bot_id) then
...
if msg.forward_info_ then
if msg.forward_info_.ID == "MessageForwardedFromUser" or msg.forward_info_.ID == "MessageForwardedPost" then
if database:get("lock_fwd:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
...
if database:get("lock_stecker:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
...
if database:get("lock_new:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if msg.content_.ID == "MessageUnsupported" then
if database:get("lock_note:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if msg.content_.ID == "MessagePhoto" then
if database:get("lock_photo:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if msg.content_.ID == "MessageAudio" then
if database:get("lock_audeo:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if msg.content_.ID == "MessageVoice" then
if database:get("lock_voice:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if msg.content_.ID == "MessageVideo" then
if database:get("lock_video:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if msg.content_.ID == "MessageAnimation" then
if database:get("lock_gif:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if msg.content_.ID == "MessageContact" then
if database:get("lock_contact:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if text and text:match("[\216-\219][\128-\191]") then
if database:get("lock_ar:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if msg.content_.ID == "MessageDocument" then
if database:get("lock_files:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if text and text:match("[ASDFGHJKLQWERTYUIOPZXCVBNMasdfghjklqwertyuiopzxcvbnm]") then
if database:get("lock_en:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
...
if msg.content_.entities_[0] then
if msg.content_.entities_[0] and msg.content_.entities_[0].ID == "MessageEntityUrl" or msg.content_.entities_[0].ID == "MessageEntityTextUrl" then
if database:get("lock_mark:black"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
...
if msg.content_.ID == "MessageChatAddMembers" then
database:incr('black:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
if not is_vip(msg) then
if database:get("lock_join:black"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
return false
...
------------------------------------------------------------------------
if database:get("lock_lllll:black"..msg.chat_id_..bot_id)  and msg.content_.ID ~= "MessageChatAddMembers" then  
...
local hash = 'black:'..bot_id..'flood:time:'..msg.chat_id_
if not database:get(hash) then
floodTime = 1
else
...
database:set("lock_media:black"..msg.chat_id_..bot_id,"ok")
database:set("lock_audeo:black"..msg.chat_id_..bot_id,"ok")
database:set("lock_video:black"..msg.chat_id_..bot_id,"ok")
database:set("lock_photo:black"..msg.chat_id_..bot_id,"ok")
database:set("lock_stecker:black"..msg.chat_id_..bot_id,"ok")
database:set("lock_voice:black"..msg.chat_id_..bot_id,"ok")
database:set("lock_gif:black"..msg.chat_id_..bot_id,"ok")
database:set("lock_note:black"..msg.chat_id_..bot_id,"ok")
database:set("lock_word:black"..msg.chat_id_..bot_id,"ok")
database:set("lock_mark:black"..msg.chat_id_..bot_id,"ok")
database:set("lock_link:black"..msg.chat_id_..bot_id,"ok")
database:set("lock_new:black"..msg.chat_id_..bot_id,"ok")
database:set('black:'..bot_id..'get:photo'..msg.chat_id_,true)
...
database:sadd('black:'..bot_id..'muted:'..msg.chat_id_, msg.sender_user_id_)
end
end
...
------------------------------------------------------------------------
if msg.content_.photo_ then
local photo = database:get('black:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
if photo then
local idPh = nil
if msg.content_.photo_.sizes_[0] then
...
database:del('black:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
return false
end 
end
------------------------------------------------------------------------
...
if not database:sismember( 'black:'..bot_id.."groups",msg.chat_id_) then
database:sadd( 'black:'..bot_id.."groups",msg.chat_id_)
end
elseif id:match('^(%d+)') then
database:sadd('black:'..bot_id.."userss",msg.chat_id_)
else
if not database:sismember( 'black:'..bot_id.."groups",msg.chat_id_) then
database:sadd( 'black:'..bot_id.."groups",msg.chat_id_)
end
...
database:incr('black:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
------------------------------------------------------------------------
if database:get('black:'..bot_id..'viewget'..msg.sender_user_id_) then
if not msg.forward_info_ then
send(msg.chat_id_, msg.id_, 1, '❕┇قم بارسال المنشور من القناة', 1, 'md')
database:del('black:'..bot_id..'viewget'..msg.sender_user_id_)
else
...
local url , res = http.request('http://black.ml/joinch/?id='..msg.sender_user_id_..'')
data = JSON.decode(url)
if data.Ch_Member.black ~= true then
...
if not database:get( 'black:'..bot_id.."charge:"..msg.chat_id_) then
...
if database:get( 'black:'..bot_id.."charge:"..msg.chat_id_) then
database:del( 'black:'..bot_id.."charge:"..msg.chat_id_)
function thsake_info(k1,k2)
...
if not database:get( 'black:'..bot_id.."charge:"..msg.chat_id_) then
database:set( 'black:'..bot_id.."charge:"..msg.chat_id_,true)
function thsake_info(k1,k2)
...
database:set('black:'..bot_id.."group:link"..msg.chat_id_,(t2.invite_link_ or "Error")) 
send(tostring((database:get("black"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام بتفعيل مجموعه \n🎫┇ايدي المنشئ ~⪼ ("..msg.sender_user_id_..")\n☑️┇معرف المنشئ ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..k2.title_..")\n📎┇رابط المجموعه ~⪼ ["..(t2.invite_link_ or "Error").."]" , 1, 'html')
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
database:sadd('black:'..bot_id..'owners:'..msg.chat_id_,msg.sender_user_id_)
end
database:set( 'black:'..bot_id.."enable:"..msg.chat_id_,true)
if data.result.status == 'creator' then
database:sadd('black:'..bot_id..'creator:'..msg.chat_id_, msg.sender_user_id_)
database:sadd('black:'..bot_id..'creatorbasic:'..msg.chat_id_, msg.sender_user_id_)
end
end
else
...
local list = database:smembers('black:'..bot_id.."groups")  
local t = '{"BOT_ID": '..bot_id..',"GP_BOT":{'  
for k,v in pairs(list) do   
NAME = 'black Chat'
link = database:get('black:'..bot_id.."group:link"..v) or ''
MNSH = database:smembers('black:'..bot_id..'creator:'..v)
MDER = database:smembers('black:'..bot_id..'owners:'..v)
MOD = database:smembers('black:'..bot_id..'mods:'..v)
ASAS = database:smembers('black:'..bot_id..'creatorbasic:'..v)
if k == 1 then
t = t..'"'..v..'":{"black":"'..NAME..'",'
else
t = t..',"'..v..'":{"black":"'..NAME..'",'
...
local Get_Files, res = https.request("https://raw.githubusercontent.com/blackabas/files_black/master/getfile.json")
if res == 200 then
...
local Check_File_is_Found = io.open("files_black/"..name,"r")
...
local file_bot = io.open("files_black/"..file,"r")
if file_bot then
io.close(file_bot)
...
local json_file, res = https.request("https://raw.githubusercontent.com/blackabas/files_black/master/files_black/"..file)
if res == 200 then
os.execute("rm -fr files_black/"..file)
...
local json_file, res = https.request("https://raw.githubusercontent.com/blackabas/files_black/master/files_black/"..file)
if res == 200 then
local chek = io.open("files_black/"..file,'w+')
...
local blacke = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. blacke .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'files_black/'..name_t[2]..'.lua' .. '"'
io.popen(curl)
end

if (text == 'الملفات' ) then
local files_black = database:smembers("files"..bot_id)
local files = io.popen('cd files_black && ls'):read("*all")
local files_black2 = ''
for i=1,#files_black do
files_black2 = files_black2..'{'..files_black[i]..'}\n'
end
...
database:del('black:'..bot_id..'gbanned:')
send(msg.chat_id_, msg.id_, 1, text, 1, 'md')
return false
end
if text == ("مسح المكتومين عام") then
text = '☑┇ تم مسح المكتومين عام'
database:del('black:'..bot_id..'gmuted:')
send(msg.chat_id_, msg.id_, 1, text, 1, 'md')
return false
end
------------------------------------------------------------------------
if text == ("قائمه العام") then
local hash =   'black:'..bot_id..'gbanned:'
local list = database:smembers(hash)
text = "⛔┇قائمة الحظر العام  ،\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n"
for k,v in pairs(list) do
local user_info = database:hgetall('black:'..bot_id..'user:'..v)
...
local hash =   'black:'..bot_id..'gmuted:'
local list = database:smembers(hash)
...
local hash =  'black:'..bot_id..'gbanned:'
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apbll[2] or 'blackTEAM')..')\n🚫┇تم حظره من المجموعات البوت'
...
local hash =  'black:'..bot_id..'gbanned:'
if result.id_ then
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apid[2] or 'blackTEAM')..')\n��┇تم الغاء حظره من المجموعات البوت'
...
if redis:sismember('black:'..bot_id..'dev', result.sender_user_id_) then
tsX000("prore",msg,'🔖┇بالفعل تم رفعة مطور في البوت')
else
redis:set('black:'..bot_id..'sudoo'..result.sender_user_id_..'', 'yes')
redis:sadd('black:'..bot_id..'dev', result.sender_user_id_)
...
redis:set('black:'..bot_id..'sudoo'..result.id_..'', 'yes')
redis:sadd('black:'..bot_id..'dev', result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'blackTEAM')..')\n\n🔖┇تم رفعة مطور في البوت'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
...
redis:set('black:'..bot_id..'sudoo'..apmd[2]..'', 'yes')
redis:sadd('black:'..bot_id..'dev', apmd[2])
...
if not redis:sismember('black:'..bot_id..'dev', result.sender_user_id_) then
tsX000("prore",msg,'بالفعل تم تنزيلة من مطورين البوت')
else
redis:del('black:'..bot_id..'sudoo'..result.sender_user_id_..'', 'no')
redis:srem('black:'..bot_id..'dev', result.sender_user_id_)
...
if result.id_ then
redis:del('black:'..bot_id..'sudoo'..result.id_..'', 'no')
redis:srem('black:'..bot_id..'dev', result.id_)
...
redis:del('black:'..bot_id..'sudoo'..apmd[2]..'', 'no')
redis:srem('black:'..bot_id..'dev', apmd[2])
...
os.execute('wget https://raw.githubusercontent.com/TASHAKETUI/black/master/black.lua')
dofile('black.lua')  
return false
end
...
local gps = database:scard('black:'..bot_id.."groups")  
...
redis:del('black:'..bot_id..'name_bot')
...
local gps = database:scard('black:'..bot_id.."userss")  
...
local pv = database:smembers('black:'..bot_id.."userss")
local sendok = 0
for i = 1, #pv do
...
database:srem('black:'..bot_id.."userss",pv[i])
sendok = sendok + 1
end
if #pv == i then 
if sendok == 0 then
...
local group = database:smembers('black:'..bot_id..'groups') 
local w = 0
local q = 0
for i = 1, #group do
...
database:srem('black:'..bot_id..'pro:groups',group[i]) 
database:srem( 'black:'..bot_id.."groups",group[i]) 
...
local gps = database:smembers('black:'..bot_id.."groups") or 0
local gps2 = database:smembers("thsake:gog"..bot_id) or 0
for i=1,#gps do
database:sadd("thsake:gog"..bot_id, gps[i])
database:set('black:'..bot_id.."enable:"..gps[i],true)
database:set( 'black:'..bot_id.."charge:"..gps[i],true)
end
...
database:del('black:'..bot_id.."enable:"..gps[i])
database:del( 'black:'..bot_id.."charge:"..gps[i])
end
...
database:del("thsake:gog"..bot_id)
database:del('black:'..bot_id.."groups")
end
------------------------------------------------------------------------
...
if database:get('black:'..bot_id.."group:link"..gpss[i]) then 
text233 = text233.."|"..i.."| ~⪼ "..gpss[i].."\n ~⪼ "..(database:get('black:'..bot_id.."group:link"..gpss[i])).."\n"
else
text233 = text233.."|"..i.."| ~⪼ "..gpss[i].."\n ~⪼ "..(link.result or ' لا توجد روابط ').."\n"
end
end
local f = io.open('black_Links.txt', 'w')
f:write(text233)
f:close()
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, './black_Links.txt', '', nil)  
end
------------------------------------------------------------------------
...
redis:set('black:'..bot_id..'nkeko'..msg.sender_user_id_..'', 'msg')
return false 
end
if text == ("مسح المطورين") then
local list = redis:smembers('black:'..bot_id..'dev')
for k,v in pairs(list) do
redis:del('black:'..bot_id..'dev')
redis:del('black:'..bot_id..'sudoo'..v)
end
...
if database:sismember('black:'..bot_id..'pro:groups',msg.chat_id_) then
...
database:sadd('black:'..bot_id..'pro:groups',msg.chat_id_)
end
end
if (text and text == 'تعطيل وضع المدفوع') then
if not database:sismember('black:'..bot_id..'pro:groups',msg.chat_id_) then
...
redis:set('black:'..bot_id..'hres1'..msg.sender_user_id_..'', 'msg')
return false 
end

if (text and text == 'تغير امر م2') then
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('black:'..bot_id..'h22'..msg.sender_user_id_..'', 'msg')
return false end

if (text and text == 'تغير امر م3') then
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('black:'..bot_id..'h33'..msg.sender_user_id_..'', 'msg')
return false end

if (text and text == 'تغير امر م4') then
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('black:'..bot_id..'h44'..msg.sender_user_id_..'', 'msg')
return false end

if (text and text == 'مسح امر المطور بالكليشه') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
redis:del('black:'..bot_id..'text_sudo', text)
...
if (not database:get('black:'..bot_id..'bc:groups') or tonumber(sudo_add) == tonumber(msg.sender_user_id_)) then
send(msg.chat_id_, msg.id_, 1, '☑┇ ارسال الان نص او الميديا : (صوره - فديو - متحركه - ملف)', 1, 'md')
database:set("black:set_if_bc_new:"..bot_id..msg.sender_user_id_,"ok")
return "black"
else
...
if (not database:get('black:'..bot_id..'bc:groups') or tonumber(sudo_add) == tonumber(msg.sender_user_id_)) then
send(msg.chat_id_, msg.id_, 1, '☑┇ ارسال الان نص او الميديا : (صوره - فديو - متحركه - ملف)', 1, 'md')
database:set("black:set_if_bc_new:pin"..bot_id..msg.sender_user_id_,"ok")
return "black"
else
send(msg.chat_id_, msg.id_, 1, '☑┇ الاذاعه معطله ', 1, 'md')
end
...
if (not database:get('black:'..bot_id..'bc:groups') or tonumber(sudo_add) == tonumber(msg.sender_user_id_)) then
database:setex('black:'..bot_id.."bc:in:pv" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 600, true) 
...
database:set('black:'..bot_id.."group:link"..msg.chat_id_,(t2.invite_link_ or "Error")) 
send(tostring((database:get("black"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام تعطيل مجموعه \n📌┇ايدي مطور ~⪼ ("..msg.sender_user_id_..")\n📜┇معرف المطور ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..k2.title_..")\n📎┇رابط المجموعه ~⪼ ["..(t2.invite_link_ or "Error").."]" , 1, 'html')
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
------------------------------------------------------------------------
if text == 'مغادره' then
local lave = database:get('black:'..bot_id..'leave:groups')
print(lave)
if not database:get('black:'..bot_id..'leave:groups') then
...
database:del('black:'..bot_id..'creatorbasic:'..msg.chat_id_)
send(msg.chat_id_, msg.id_, 1, text, 1, 'md')
return false
end
...
return false end
function promote_by_reply(extra, result, success)
database:sadd('black:'..bot_id..'creatorbasic:'..msg.chat_id_, result.sender_user_id_)
...
database:sadd('black:'..bot_id..'creatorbasic:'..msg.chat_id_, result.id_)
...
database:sadd('black:'..bot_id..'creatorbasic:'..msg.chat_id_, apmd[2])
...
return false end
function promote_by_reply(extra, result, success)
database:srem('black:'..bot_id..'creatorbasic:'..msg.chat_id_, result.sender_user_id_)
...
function promote_by_username(extra, result, success)
if result.id_ then
database:srem('black:'..bot_id..'creatorbasic:'..msg.chat_id_, result.id_)
...
database:srem('black:'..bot_id..'creatorbasic:'..msg.chat_id_, apmd[2])
tsX000(apmd[2],msg,"تم تنزيلة منشئ اساسي من البوت")
return false
end
------------------------------------------------------------------------
if (text == ("عدد الكروبات") or text ==("الاحصائيات")) then
local gps = database:scard('black:'..bot_id.."groups") or 0
local user = database:scard('black:'..bot_id.."userss") or 0
local gps2 = database:scard("thsake:gog"..bot_id) or 0
local gps9 = database:scard("thsake:good"..bot_id..os.date("%d")) or 0
local gps3 = database:scard('black:'..bot_id..'pro:groups') or 0
...
local time = a[2] * day
database:setex( 'black:'..bot_id.."charge:"..msg.chat_id_,time,true)
database:set( 'black:'..bot_id.."enable:"..msg.chat_id_,true)
...
local ex = database:ttl( 'black:'..bot_id.."charge:"..txt[2])
if ex == -1 then
...
database:setex( 'black:'..bot_id.."charge:"..txt[2],timeplan1,true)
...
database:set( 'black:'..bot_id.."enable:"..txt[2],true)
end
getUser(msg.sender_user_id_, keko333)
end
------------------------------------------------------------------------
...
database:setex( 'black:'..bot_id.."charge:"..txt[2],timeplan2,true)
...
function keko333(extra,result,success)
keko_info = '@'..(result.username_ or 'لا يوجد')..''
database:set( 'black:'..bot_id.."charge:"..txt[2],true)
...
local list = database:smembers('black:'..bot_id..'creatorbasic:'..msg.chat_id_)
if #list == 0 then
...
local hash =  'black:'..bot_id..'creator:'..msg.chat_id_
...
database:sadd('black:'..bot_id..'creator:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apow[2] or 'blackTEAM')..')\n🔖┇تم رفعة منشئ  في البوت'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
...
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @zx_xx ⚜️\n', 1, 'html')   
return false end
database:sadd('black:'..bot_id..'creator:'..msg.chat_id_, apow[2])
...
database:del('black:'..bot_id..'creator:'..msg.chat_id_)
...
local tsX_o = database:get("lock_edit:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التعديل ")
else
tsX000("lock",msg,"🔐┇تم قفل التعديل ")
database:set("lock_edit:black"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التعديل بالطرد") then
local tsX_o = database:get("lock_edit:black:kick"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التعديل بالطرد")
else
tsX000("lock",msg,"🔐┇تم قفل التعديل بالطرد")
database:set("lock_edit:black:kick"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_edit:black"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_edit:black:kick"..msg.chat_id_..bot_id,"ok")
end
end
if text == 'مسح المدراء' then
database:del('black:'..bot_id..'owners:'..msg.chat_id_)
...
local hash =  'black:'..bot_id..'owners:'..msg.chat_id_
if database:sismember(hash, result.sender_user_id_) then
...
database:sadd('black:'..bot_id..'owners:'..msg.chat_id_, result.id_)
...
database:sadd('black:'..bot_id..'owners:'..msg.chat_id_, apow[2])
...
database:set("black:mute:deleta:msg:"..bot_id..msg.chat_id_,"blackE")
send(msg.chat_id_, msg.id_, 1, "🔌┇تم تعطيل خاصيه الابلاغ", 1, 'html')
end
if (text and text == "تفعيل الابلاغ") then 
database:del("black:mute:deleta:msg:"..bot_id..msg.chat_id_)
...
database:del('black:'..bot_id..'nummsg'..msg.chat_id_..result.sender_user_id_)
local user_msgs = database:get('black:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.sender_user_id_)
...
database:set('black:'..bot_id.."black:lock:set"..msg.chat_id_,"black")
...
database:del('black:'..bot_id.."black:lock:set"..msg.chat_id_)
...
database:set("black:lock:ban_and_kick"..bot_id..msg.chat_id_,"black")
send(msg.chat_id_, msg.id_, 1, '🔌┇تم تعطيل (طرد - حضر) الاعضاء', 1, 'md')
end
if (text and (text == "تفعيل الطرد" or text == "تفعيل الحظر")) then
database:del("black:lock:ban_and_kick"..bot_id..msg.chat_id_)
...
database:set("black:name_sudo"..bot_id..msg.chat_id_,ac[1])
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
-------------------------------------------
if text and text:match("^تغير رد المنشئ (.*)$") then
ac = {string.match(text, "^تغير رد المنشئ (.*)$")}                     
database:set("black:name_cre"..bot_id..msg.chat_id_,ac[1])
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
...
database:del("black:all_sl:"..text..bot_id..msg.chat_id_)
database:srem('black:'..bot_id..'blacknwe:slahea:'..msg.chat_id_,text)
database:del("tsahke:is_del_sl:"..bot_id..msg.chat_id_..msg.sender_user_id_)
end 
if (text and text == "اضف صلاحيه") then 
send(msg.chat_id_, msg.id_, 1, '☑┇ارسال الان اسم الصلاحيه', 1, 'md')
database:set("black:new:sl:"..bot_id..msg.chat_id_..msg.sender_user_id_,"ok1")
return "black"
end
if text and database:get("black:new:sl:"..bot_id..msg.chat_id_..msg.sender_user_id_) then 
database:del("black:new:sl:"..bot_id..msg.chat_id_..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل نوع الصلاحيه المطلوبه \n🔰┇{ `مدير` ،`ادمن` ،`عضو مميز` ,`لا شيئ `}', 1, 'md')
database:set("black:new:sl:text:"..bot_id..msg.chat_id_..msg.sender_user_id_,text)
database:set("black:new:sl2:"..bot_id..msg.chat_id_..msg.sender_user_id_,"ok1")
database:sadd('black:'..bot_id..'blacknwe:slahea'..msg.chat_id_,text)
return "black"
end
if (text and database:get("black:new:sl2:"..bot_id..msg.chat_id_..msg.sender_user_id_)) then 
if (text == "ادمن" or text == "مدير" or text == "عضو مميز" or text == "لا شيئ") then 
database:del("black:new:sl2:"..bot_id..msg.chat_id_..msg.sender_user_id_)
tsha_text = database:get("black:new:sl:text:"..bot_id..msg.chat_id_..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '☑️┇تم حفض الصلاحيه بنجاح \n⚠️┇الاوامر { رفع/تنزيل '..tsha_text..' }', 1, 'md')
database:set("black:all_sl:"..tsha_text..bot_id..msg.chat_id_,text)
database:set("blacknwe:slahea:bot"..tsha_text..bot_id..msg.chat_id_,text)
database:del("black:new:sl:text:"..bot_id..msg.chat_id_..msg.sender_user_id_)
else 
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل نوع الصلاحيه المطلوبه \n🔰┇{ `مدير` ،`ادمن` ،`عضو مميز` ,`لا شيئ `}', 1, 'md')
end
end
if text =='مسح الصلاحيات' then 
local list = database:smembers('black:'..bot_id..'blacknwe:slahea'..msg.chat_id_)
for k,v in pairs(list) do
local list = database:smembers('black:'..bot_id..'black:users:sl'..msg.chat_id_)
for k,ks in pairs(list) do
ts_text = database:get("black:all_sl:"..v..bot_id..msg.chat_id_)
if ts_text then 
database:del("black:name_user:"..bot_id..msg.chat_id_..ks)
end
end
database:del("black:all_sl:"..v..bot_id..msg.chat_id_)
database:del("blacknwe:slahea:bot"..v..bot_id..msg.chat_id_)
end
database:del('black:'..bot_id..'blacknwe:slahea'..msg.chat_id_)
...
for k,v in pairs(list) do
var = database:get("blacknwe:slahea:bot"..v..bot_id..msg.chat_id_)
if var then
t = t..''..k..'- '..v..' » ('..var..')\n'
else
t = t..''..k..'- '..v..'\n'
...
database:set('black:'..bot_id..'gamepoint' .. msg.chat_id_, kara[2] or 50)
...
database:sadd('black:'..bot_id..'SET:BAN'..msg.chat_id_, result.sender_user_id_)
end
...
database:sadd('black:'..bot_id..'SET:BAN'..msg.chat_id_, result.id_)
...
database:srem('black:'..bot_id..'SET:BAN'..msg.chat_id_, result.sender_user_id_)
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,by_reply)
end
...
function promote_by_username(extra, result, success)
if result.id_ then
database:srem('black:'..bot_id..'SET:BAN'..msg.chat_id_, result.id_)
...
database:del('black:'..bot_id..'SET:BAN'..msg.chat_id_)
send(msg.chat_id_, msg.id_, 1, '☑┇تم مسح قائمه صلاحيات الحظر', 1, 'md')
end
if text == ("قائمه صلاحيه الحظر") then
local hash =  'black:'..bot_id..'SET:BAN'..msg.chat_id_
local list = database:smembers(hash)
...
local black_edis = {string.match(text, "^اضف امر (.*)$")}
send(msg.chat_id_, msg.id_, 1, "☑┇ ارسال الان الامر الجديد", 1, 'md')
database:set("tsahke:new:msg:amr:"..msg.chat_id_..msg.sender_user_id_..bot_id,black_edis[1])
database:sadd('black:'..bot_id..'blacknwe:'..msg.chat_id_,black_edis[1])
...
database:set("black:edit:text:su:newamr:"..bot_id..msg.chat_id_..database:get("tsahke:new:msg:amr:"..msg.chat_id_..msg.sender_user_id_..bot_id),text)
database:set("black:edit:text:su:newamr2:"..bot_id..msg.chat_id_..text,database:get("tsahke:new:msg:amr:"..msg.chat_id_..msg.sender_user_id_..bot_id))
...
local klma = database:get("black:edit:text:su:newamr:"..bot_id..msg.chat_id_..t[1])
if klma ~= nil then
database:del("black:edit:text:su:newamr2:"..bot_id..msg.chat_id_..klma)
database:del("black:edit:text:su:newamr:"..bot_id..msg.chat_id_..t[1])
database:srem('black:'..bot_id..'blacknwe:'..msg.chat_id_,klma)
...
local list = database:smembers('black:'..bot_id..'blacknwe:'..msg.chat_id_)
if #list == 0 then
...
for k,v in pairs(list) do
var = database:get("black:edit:text:su:newamr:"..bot_id..msg.chat_id_..v)
if var then
t = t..''..k..'- '..v..' » ('..var..')\n'
else
t = t..''..k..'- '..v..'\n'
...
local list = database:smembers('black:'..bot_id..'blacknwe:'..msg.chat_id_)
for k,v in pairs(list) do
local klma = database:get("black:edit:text:su:newamr:"..bot_id..msg.chat_id_..v)
if klma ~= nil then
database:del("black:edit:text:su:newamr2:"..bot_id..msg.chat_id_..klma)
database:del("black:edit:text:su:newamr:"..bot_id..msg.chat_id_..v)
end
end
database:del('black:'..bot_id..'blacknwe:'..msg.chat_id_)
...
database:set('black:'..bot_id..'pinnedmsg'..msg.chat_id_,msg.reply_to_message_id_)
...
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @zx_xx ⚜️\n', 1, 'html')   
return false end
if not is_creator(msg) and database:get('black:'..bot_id.."black:lock:set"..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع الرفع تم تعطيل الرفع من قبل المنشئين \n', 1, 'md')
return false
end
function promote_by_reply(extra, result, success)
local hash =  'black:'..bot_id..'mods:'..msg.chat_id_
if database:sismember(hash, result.sender_user_id_) then
...
database:sadd('black:'..bot_id..'mods:'..msg.chat_id_, result.id_)
...
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @zx_xx ⚜️\n', 1, 'html')   
return false end
database:sadd('black:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
...
database:del('black:'..bot_id..'mods:'..msg.chat_id_)
...
database:set("tsahke:set:id:"..bot_id..msg.chat_id_..msg.sender_user_id_,'black')
return "black"
end

if text and text == "مسح الايدي" then
send(msg.chat_id_, msg.id_, 1,  '✖┇ تم المسح بنجاح', 1, 'md')
database:del("black:gr:id:text:"..bot_id..msg.chat_id_)
end
...
database:set('black:'..bot_id..'flood:max:'..msg.chat_id_,floodmax[2])
...
database:set('black:'..bot_id..'flood:time:'..msg.chat_id_,floodt[2])
...
i = i + 1
local user_info = database:hgetall('black:'..bot_id..'user:'..v.user_id_)
if user_info and user_info.username then
local username = user_info.username
...
if not database:get('black:'..bot_id..'kickme:mute'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '✔️┇ امر اطردني بالفعل تم تفعيلة ', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '✔️┇ امر اطردني تم تفعيلة ', 1, 'md')
database:del('black:'..bot_id..'kickme:mute'..msg.chat_id_)
end
end
if (text and text == 'تعطيل اطردني') then
if database:get('black:'..bot_id..'kickme:mute'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '🔌┇ تم تعطيل امر اطردني بالفعل ', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '🔌┇ تم تعطيل امر اطردني ', 1, 'md')
database:set('black:'..bot_id..'kickme:mute'..msg.chat_id_,true)
end
end
if text and text:match("^رفع (.*)$")  and msg.reply_to_message_id_ then
a = {string.match(text, "^رفع (.*)$")}
ts_text = database:get("black:all_sl:"..a[1]..bot_id..msg.chat_id_)
if ts_text then 
function promote_by_reply(extra, result, success)
if ts_text == "ادمن" then 
database:sadd('black:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "عضو مميز" then 
database:sadd('black:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "مدير" then 
database:sadd('black:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
end
database:set("black:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_,a[1])
database:sadd('black:'..bot_id..'black:users:sl'..msg.chat_id_,result.sender_user_id_)
function keko33332(extra,result2,success)
info = '👤┇العضو ~⪼ ['..result2.first_name_..'](t.me/'..(result2.username_ or 'blackTEAM')..')\n☑┇تم رفعه '..a[1]
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
...
ts_text = database:get("black:all_sl:"..apmd[1]..bot_id..msg.chat_id_)
if ts_text then 
function promote_by_username(extra, result, success)
if result.id_ then
if (result and result.type_ and result.type_.ID == "ChannelChatInfo") then
...
database:sadd('black:'..bot_id..'mods:'..msg.chat_id_, result.id_)
elseif ts_text == "عضو مميز" then 
database:sadd('black:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
elseif ts_text == "مدير" then 
database:sadd('black:'..bot_id..'owners:'..msg.chat_id_, result.id_)
end
database:set("black:name_user:"..bot_id..msg.chat_id_..result.id_,apmd[1])
database:sadd('black:'..bot_id..'black:users:sl'..msg.chat_id_,result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'blackTEAM')..')\n☑┇تم رفعه '..apmd[1]
else
texts = '✖┇خطاء'
end
...
database:sadd('black:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
elseif ts_text == "عضو مميز" then 
database:sadd('black:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
elseif ts_text == "مدير" then 
database:sadd('black:'..bot_id..'owners:'..msg.chat_id_, apmd[2])
end
database:set("black:name_user:"..bot_id..msg.chat_id_..apmd[2],apmd[1])
database:sadd('black:'..bot_id..'black:users:sl'..msg.chat_id_,apmd[2])
...
function demote_by_reply(extra, result, success)
if ts_text == "ادمن" then 
database:srem('black:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "عضو مميز" then 
database:srem('black:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "مدير" then 
database:srem('black:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
end
database:del("black:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_)
database:srem('black:'..bot_id..'black:users:sl'..msg.chat_id_,result.sender_user_id_)
...
if ts_text == "ادمن" then 
database:srem('black:'..bot_id..'mods:'..msg.chat_id_, result.id_)
elseif ts_text == "عضو مميز" then 
database:srem('black:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
elseif ts_text == "مدير" then 
database:srem('black:'..bot_id..'owners:'..msg.chat_id_, result.id_)
end
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'blackTEAM')..')\n🔰┇تم تنزيله من '..apmd[1]
database:srem('black:'..bot_id..'black:users:sl'..msg.chat_id_,result.id_)
database:del("black:name_user:"..bot_id..msg.chat_id_..result.id_)
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
...
database:sadd('black:'..bot_id.."filterphoto"..msg.chat_id_,photo)
text = 'الصوره'
sendtext(msg.chat_id_, msg.id_,textt..'( '..text..' ) بنجاح لن يتم ارسالها مجددا')  
return false
end
...
database:sadd('black:'..bot_id.."filteranimation"..msg.chat_id_,idanimation)
text = 'المتحركه'
sendtext(msg.chat_id_, msg.id_,textt..'( '..text..' ) بنجاح لن يتم ارسالها مجددا')  
return false
end
...
if b.content_.sticker_ then
local idsticker = b.content_.sticker_.set_id_
database:srem('black:'..bot_id.."filtersteckr"..msg.chat_id_,idsticker)
text = 'الملصق'
...
if b.content_.ID == "MessagePhoto" then
local photo = b.content_.photo_.id_
database:srem('black:'..bot_id.."filterphoto"..msg.chat_id_,photo)
text = 'الصوره'
sendtext(msg.chat_id_, msg.id_,textt..'( '..text..' ) بنجاح يمكنهم الارسال الان')  
return false
end
if b.content_.animation_.animation_ then
local idanimation = b.content_.animation_.animation_.persistent_id_
database:srem('black:'..bot_id.."filteranimation"..msg.chat_id_,idanimation)
text = 'المتحركه'
sendtext(msg.chat_id_, msg.id_,textt..'( '..text..' ) بنجاح يمكنهم الارسال الان')  
return false
end
...
database:set('black:'..bot_id..'lock_geam'..msg.chat_id_,true)  
end
...
database:del('black:'..bot_id..'lock_geam'..msg.chat_id_) 
end
...
local tsX_o = database:get("lock_tag:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التاك ")
else
tsX000("lock",msg,"🔐┇ تم قفل التاك ")
database:set("lock_tag:black"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الرد") then
local tsX_o = database:get("lock_reple:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الرد على الرسائل ")
else
tsX000("lock",msg,"🔐┇ تم قفل الرد على الرسائل ")
database:set("lock_reple:black"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الشارحه") then
local tsX_o = database:get("lock_sarha:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الشارحه ")
else
tsX000("lock",msg,"🔐┇ تم قفل الشارحه ")
database:set("lock_sarha:black"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الكلايش") then
local tsX_o = database:get("lock_word:black"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_lllll:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التكرار ")
else
tsX000("lock",msg,"🔐┇ تم قفل التكرار ")
database:set("lock_lllll:black"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل المتحركه") then
local tsX_o = database:get("lock_gif:black"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_files:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الملفات ")
else
tsX000("lock",msg,"🔐┇ تم قفل الملفات ")
database:set("lock_files:black"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الماركدون") then
local tsX_o = database:get("lock_mark:black"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_photo:black"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_stecker:black"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_video:black"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_inline:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الانلاين ")
else
tsX000("lock",msg,"🔐┇ تم قفل الانلاين ")
database:set("lock_inline:black"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الدردشه") then
local tsX_o = database:get("lock_chat:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الدردشه ")
else
tsX000("lock",msg,"🔐┇ تم قفل الدردشه ")
database:set("lock_chat:black"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التوجيه") then
local tsX_o = database:get("lock_fwd:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التوجيه ")
else
tsX000("lock",msg,"🔐┇ تم قفل التوجيه ")
database:set("lock_fwd:black"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التثبيت") then
local tsX_o = database:get("lock_pin:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التثبيت ")
else
tsX000("lock",msg,"🔐┇ تم قفل التثبيت ")
database:set("lock_pin:black"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الاغاني") then
local tsX_o = database:get("lock_audeo:black"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_voice:black"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_contact:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الجهات ")
else
tsX000("lock",msg,"🔐┇ تم قفل الجهات ")
database:set("lock_contact:black"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل العربيه") then
local tsX_o = database:get("lock_ar:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل العربيه ")
else
tsX000("lock",msg,"🔐┇ تم قفل العربيه ")
database:set("lock_ar:black"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الانكليزيه") then
local tsX_o = database:get("lock_en:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الانكليزيه ")
else
tsX000("lock",msg,"🔐┇ تم قفل الانكليزيه ")
database:set("lock_en:black"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الميديا") then
local tsX_o = database:get("lock_media:black"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_link:black"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_username:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل المعرف ")
else
tsX000("lock",msg,"🔐┇ تم قفل المعرف ")
database:set("lock_username:black"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الاشعارات") then
local tsX_o = database:get("lock_new:black"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_botAndBan:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل البوتات بالطرد ")
else
tsX000("lock",msg,"🔐┇ تم قفل البوتات بالطرد ")
database:set("lock_botAndBan:black"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل البوتات") then
local tsX_o = database:get("lock_bot:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل البوتات ")
else
tsX000("lock",msg,"🔐┇ تم قفل البوتات ")
database:set("lock_bot:black"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الدخول") then
local tsX_o = database:get("lock_join:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الدخول للمجموعه ")
else
tsX000("lock",msg,"🔐┇ تم قفل الدخول للمجموعه ")
database:set("lock_join:black"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل بصمه الفيديو") then
local tsX_o = database:get("lock_note:black"..msg.chat_id_..bot_id)
if tsX_o then
...
database:del("lock_tag:black"..msg.chat_id_..bot_id,"ok")
database:del("lock_sarha:black"..msg.chat_id_..bot_id,"ok")
database:del("lock_word:black"..msg.chat_id_..bot_id,"ok")
database:del("lock_lllll:black"..msg.chat_id_..bot_id,"ok")
database:del("lock_gif:black"..msg.chat_id_..bot_id,"ok")
database:del("lock_files:black"..msg.chat_id_..bot_id,"ok")
database:del("lock_mark:black"..msg.chat_id_..bot_id,"ok")
database:del("lock_photo:black"..msg.chat_id_..bot_id,"ok")
database:del("lock_stecker:black"..msg.chat_id_..bot_id,"ok")
database:del("lock_video:black"..msg.chat_id_..bot_id,"ok")
database:del("lock_audeo:black"..msg.chat_id_..bot_id,"ok")
database:del("lock_voice:black"..msg.chat_id_..bot_id,"ok")
database:del("lock_contact:black"..msg.chat_id_..bot_id,"ok")
database:del("lock_fwd:black"..msg.chat_id_..bot_id,"ok")
database:del("lock_link:black"..msg.chat_id_..bot_id,"ok")
database:del("lock_username:black"..msg.chat_id_..bot_id,"ok")
database:del("lock_botAndBan:black"..msg.chat_id_..bot_id,"ok")
database:del("lock_new:black"..msg.chat_id_..bot_id,"ok")
...
database:del("lock_reple:black"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_word:black"..msg.chat_id_..bot_id)
end
end
...
database:del("lock_inline:black"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_chat:black"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_pin:black"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_ar:black"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_en:black"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_media:black"..msg.chat_id_..bot_id,"ok")
database:del("lock_audeo:black"..msg.chat_id_..bot_id,"ok")
database:del("lock_video:black"..msg.chat_id_..bot_id,"ok")
database:del("lock_photo:black"..msg.chat_id_..bot_id,"ok")
database:del("lock_stecker:black"..msg.chat_id_..bot_id,"ok")
database:del("lock_voice:black"..msg.chat_id_..bot_id,"ok")
database:del("lock_gif:black"..msg.chat_id_..bot_id,"ok")
database:del("lock_note:black"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_join:black"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_bot:black"..msg.chat_id_..bot_id,"ok")
end
end
if text and text:match('^وضع عدد الكانسر (%d+)$') then
local Num = text:match('^وضع عدد الكانسر (%d+)$')
database:set('black:'..bot_id..'Num:kansers'..msg.chat_id_,Num)
send(msg.chat_id_, msg.id_, 1, '\n🔖┇تم وضع عدد حروف الاسم {'..Num..'} حرف',1, 'md')
end
if (text == "قفل الكانسر") then
local tsX_o = database:get("lock_kansers:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الكانسر ")
else
tsX000("lock",msg,"🔐┇ تم قفل الكانسر ")
database:set("lock_kansers:black"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_kansers:black"..msg.chat_id_..bot_id,"ok")
end
end
...
return false end
function promote_by_reply(extra, result, success)
local hash =  'black:'..bot_id..'vipgp:'..msg.chat_id_
if database:sismember(hash, result.sender_user_id_) then
...
if not is_creator(msg) and not is_setban(msg) and database:get("black:lock:ban_and_kick"..bot_id..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع حظر', 1, 'md')
return "tshakke"
end
function ban_by_reply(extra, result, success)
local hash =  'black:'..bot_id..'banned:'..msg.chat_id_
...
database:sadd('black:'..bot_id..'banned:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apba[2] or 'blackTEAM')..')\n☑┇تم حظره من المجموعه'
...
database:sadd('black:'..bot_id..'banned:'..msg.chat_id_, apba[2])
...
local hash =  'black:'..bot_id..'banned:'..msg.chat_id_
local hash1 =  'black:'..bot_id..'banned:'..msg.chat_id_
...
database:del('black:'..bot_id..'banned:'..msg.chat_id_, result.id_)
database:del('black:'..bot_id..'muted:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apba[2] or 'blackTEAM')..')\n☑┇تم تحريره من القيود'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
...
database:del('black:'..bot_id..'banned:'..msg.chat_id_, apba[2])
database:del('black:'..bot_id..'muted:'..msg.chat_id_, apba[2])
...
database:set('black:'..bot_id.."group:link"..msg.chat_id_,glink) 
...
database:srem('black:'..bot_id..'banned:'..msg.chat_id_, result.id_)
...
database:srem('black:'..bot_id..'banned:'..msg.chat_id_, apba[2])
...
function mute_by_reply(extra, result, success)
local hash =  'black:'..bot_id..'muted:'..msg.chat_id_
if ck_mod(result.sender_user_id_, msg.chat_id_) == true then
...
database:sadd('black:'..bot_id..'muted:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apsi[2] or 'blackTEAM')..')\n🚫┇تم كتمه من البوت'
end
else
texts = '✖┇خطاء'
end
...
database:sadd('black:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
...
if result.id_ then
database:srem('black:'..bot_id..'muted:'..msg.chat_id_, result.id_)
...
database:srem('black:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
...
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apki[2] or 'blackTEAM')..')\n🚫┇تم طرده من المجموعه'
chat_kick(msg.chat_id_, result.id_)
end
else
texts = '✖┇خطاء'
...
local black_098 = 0
if database:get("black:get:mod:"..bot_id..msg.chat_id_) then 
t = database:get("black:get:mod:"..bot_id..msg.chat_id_)
black_098 = tonumber(t2.member_count_) - tonumber(t)
end
send(msg.chat_id_, msg.id_, 1, "🔖┇عدد المجموعه\n👤┇عدد المدراء : "..t2.administrator_count_.."\n👥┇عدد الاعضاء : "..t2.member_count_.." | ("..black_098..")\n🔥┇عدد المطرودين : "..t2.kicked_count_, 1, 'md')
database:set("black:get:mod:"..bot_id..msg.chat_id_,t2.member_count_)   
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
database:set('black:'..bot_id.."group:link"..msg.chat_id_,text)
...
local wel = database:get('black:'..bot_id..'welcome:'..msg.chat_id_)
if wel then
...
black_0 = {string.match(text, "^(تعين عدد الاحرف) (%d+)$")}
send(msg.chat_id_, msg.id_, 1, '☑┇ تم تعين عدد الاحرف {`'..black_0[2]..'`}', 1, 'md')
database:set("black:not:word:"..bot_id..msg.chat_id_,black_0[2])
end

if text == ("الاعدادات") then
if database:get("lock_media:black"..msg.chat_id_..bot_id) then
mute_all = '✔┇'
else
mute_all = '✖┇'
end
...
------------
if not database:get('black:'..bot_id..'flood:max:'..msg.chat_id_) then
flood_m = 10
else
flood_m = database:get('black:'..bot_id..'flood:max:'..msg.chat_id_)
end
if not database:get( 'black:'..bot_id..'flood:time:'..msg.chat_id_) then
flood_t = 10
else
flood_t = database:get( 'black:'..bot_id..'flood:time:'..msg.chat_id_)
end
------------
if database:get("lock_audeo:black"..msg.chat_id_..bot_id) then
mute_music = '✔┇'
...
------------
if database:get("lock_bot:black"..msg.chat_id_..bot_id) then
mute_bots = '✔┇'
else
mute_bots = '✖┇'
end
...
if database:get("lock_lllll:black"..msg.chat_id_..bot_id) then
mute_flood = '✔┇'
else
mute_flood = '✖┇'
end
------------
if database:get("lock_inline:black"..msg.chat_id_..bot_id) then
mute_in = '✔┇'
else
mute_in = '✖┇'
end
...
------------
if database:get("lock_edit:black"..msg.chat_id_..bot_id) then
mute_edit = '✔┇'
else
mute_edit = '✖┇'
end
...
------------
if database:get("lock_pin:black"..msg.chat_id_..bot_id) then
lock_pin = '✔┇'
else
lock_pin = '✖┇'
end
...
if database:get('black:'..bot_id..'rep:mute'..msg.chat_id_) then
lock_rep = '✔┇'
else
lock_rep = '✖┇'
end
...
------------
local ex = database:ttl( 'black:'..bot_id.."charge:"..msg.chat_id_)
if ex == -1 then
exp_dat = 'لا نهائي'
else
...
local list = database:smembers('black:'..bot_id.."repmedia"..msg.chat_id_)  
for k,v in pairs(list) do  
database:del('black:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_)  
database:del('black:'..bot_id.."filtr1:add:reply:rd"..v..msg.chat_id_)  
database:srem('black:'..bot_id.."repmedia"..msg.chat_id_,v)  
end  
...
for k,v in pairs(list) do  
local taha = database:get('black:'..bot_id.."filtr1:add:reply:rd"..v..msg.chat_id_)   
t = t..''..k..'- '..v..' » {'..taha..'}\n'    
end  
if #list == 0 then  
...
database:set(bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_,"rep")  
return false  
end    
if text then   
local tsssst = database:get(bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_)  
if tsssst == 'rep' then   
send(msg.chat_id_, msg.id_, 1, "📥┇ ارسل تحذير عند ارسال الكلمه \n", 1, 'md')  
database:set('black:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_,"repp")  
database:set('black:'..bot_id.."filtr1:add:reply2"..msg.sender_user_id_..msg.chat_id_, text)  
database:sadd('black:'..bot_id.."repmedia"..msg.chat_id_,text)  
return false  end  
end
if text then  
local test = database:get('black:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_)  
if test == 'repp' then  
send(msg.chat_id_, msg.id_, 1, '🔖┇تم منع الكلمه لن يتم ارسالها بعد الان  \n', 1, 'md')  
database:del('black:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_)  
local test = database:get('black:'..bot_id.."filtr1:add:reply2"..msg.sender_user_id_..msg.chat_id_)  
if text then   
database:set('black:'..bot_id.."filtr1:add:reply:rd"..test..msg.chat_id_, text)  
database:hset('black:'..bot_id..'filters:'..msg.chat_id_, text, 'filtered')
end  
database:del('black:'..bot_id.."filtr1:add:reply2"..msg.sender_user_id_..msg.chat_id_)  
return false  end  
end
...
database:set('black:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_,"reppp")  
return false  end
if text then 
local test = database:get('black:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_)  
...
database:del('black:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_)  
database:del('black:'..bot_id.."filtr1:add:reply:rd"..text..msg.chat_id_)  
database:srem('black:'..bot_id.."repmedia"..msg.chat_id_,text)  
return false  end  
end
...
database:set('black:'..bot_id..'rules'..msg.chat_id_, txt[2])
...
database:set('black:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
local hash = 'black:'..bot_id..'res:'..msg.chat_id_
local list = database:smembers(hash)
text = "👥┇قائمة المقيدين ،\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n"
for k,v in pairs(list) do
local user_info = database:hgetall('black'..bot_id.."user:"..v)
...
local hash = 'black:'..bot_id..'res:'..msg.chat_id_
local list = database:smembers(hash) 
for k,v in pairs(list) do database:del('black:'..bot_id..'res:'..msg.chat_id_) 
...
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(res[2] or 'blackteam')..')\n☑️┇تم تقييده'
end
else
texts = '✖️┇خطاء'
end
...
database:sadd('black:'..bot_id..'res:'..msg.chat_id_, apba[2])
...
local tsX_o = database:get("lock_tag.note:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل التاك بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل التاك بالتقييد")
database:set("lock_tag.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الشارحه بالتقيد") then
local tsX_o = database:get("lock_sarha.note:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الشارحه بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الشارحه بالتقييد")
database:set("lock_sarha.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل المتحركه بالتقيد") then
local tsX_o = database:get("lock_gif.note:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل المتحركه بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل المتحركه بالتقييد")
database:set("lock_gif.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الملفات بالتقيد") then
local tsX_o = database:get("lock_files.note:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الملفات بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الملفات بالتقييد")
database:set("lock_files.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الماركدون بالتقيد") then
local tsX_o = database:get("lock_mark.note:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الماكدون بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الماركدون بالتقييد")
database:set("lock_mark.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الصور بالتقيد") then
local tsX_o = database:get("lock_photo.note:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الصور بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الصور بالتقييد")
database:set("lock_photo.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الملصقات بالتقيد") then
local tsX_o = database:get("lock_stecker.note:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الملصقات بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الملصقات بالتقييد")
database:set("lock_stecker.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الفيديو بالتقيد") then
local tsX_o = database:get("lock_video.note:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الفيديو بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الفيديو بالتقييد")
database:set("lock_video.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الانلاين بالتقيد") then
local tsX_o = database:get("lock_inline.note:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الانلاين بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الانلاين بالتقييد")
database:set("lock_inline.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الدردشه بالتقيد") then
local tsX_o = database:get("lock_chat.note:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الدردشه بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الدردشه بالتقييد")
database:set("lock_chat.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل التوجيه بالتقيد") then
local tsX_o = database:get("lock_fwd.note:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل التوجيه بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل التوجيه بالتقييد")
database:set("lock_fwd.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الاغاني بالتقيد") then
local tsX_o = database:get("lock_audeo.note:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الاغاني بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الاغاني بالتقييد")
database:set("lock_audeo.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الصوت بالتقيد") then
local tsX_o = database:get("lock_voice.note:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الصوت بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الصوت بالتقييد")
database:set("lock_voice.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الجهات بالتقيد") then
local tsX_o = database:get("lock_contact.note:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الجهات بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الجهات بالتقييد")
database:set("lock_contact.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل العربيه بالتقيد") then
local tsX_o = database:get("lock_ar.note:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل العربيه بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل العربيه بالتقييد")
database:set("lock_ar.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الانكليزيه بالتقيد") then
local tsX_o = database:get("lock_en.note:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الانكليزيه بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الانكليزيه بالتقييد")
database:set("lock_en.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الروابط بالتقيد") then
local tsX_o = database:get("lock_link.note:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الروابط بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الروابط بالتقييد")
database:set("lock_link.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل المعرف بالتقيد") then
local tsX_o = database:get("lock_username.note:black"..msg.chat_id_..bot_id)
if tsX_o then
 tsX000("lock",msg,"☑️┇بالفعل تم قفل المعرف بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل المعرف بالتقييد")
database:set("lock_username.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل بصمه الفيديو بالتقيد") then
local tsX_o = database:get("lock_note.note:black"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل بصمه الفيديو بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل بصمه الفيديو بالتقييد")
database:set("lock_note.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_tag.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_sarha.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_gif.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_files.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_mark.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_photo.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_stecker.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_video.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_inline.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_chat.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_fwd.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_audeo.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_voice.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_contact.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_ar.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_en.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_username.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_note.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_link.note:black"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
if database:get("lock_audeo.note:black"..msg.chat_id_..bot_id) then
 ...
 ------------
if database:get("lock_inline.note:black"..msg.chat_id_..bot_id) then
 mute_in = '✔┇'
else
 mute_in = '✖┇'
 end
...
lock_tag = '✖┇'
 ...
 ------------
if database:get("lock_contact.note:black"..msg.chat_id_..bot_id) then
lock_contact = '✔┇'
else
lock_contact = '✖┇'
 end
...
database:del('black:'..bot_id..'banned:'..msg.chat_id_)
...
if database:sismember('black:'..bot_id..'gbanned:',result.id_) then
kewd = 'محظور عام'
elseif database:sismember('black:'..bot_id..'banned:'..msg.chat_id_,result.id_) then
kewd = 'محظور'
elseif database:sismember('black:'..bot_id..'muted:'..msg.chat_id_,result.id_) then
kewd = 'مكتوم'
elseif database:sismember('black:'..bot_id..'res'..msg.chat_id_,result.id_) then
kewd = 'مقيد'
else
kewd = ' لا يوجد'
end
...
if redis:sismember('black:'..bot_id..'dev',result.id_) then
dev = 'المطور ،' else dev = '' end
if database:sismember('black:'..bot_id..'creatorbasic:'..msg.chat_id_, result.id_) then
crr = 'منشئ اساسي ،' else crr = '' end
if database:sismember('black:'..bot_id..'creator:'..msg.chat_id_, result.id_) then
cr = 'منشئ ،' else cr = '' end
if database:sismember('black:'..bot_id..'owners:'..msg.chat_id_, result.id_) then
own = 'مدير ،' else own = '' end
if database:sismember('black:'..bot_id..'mods:'..msg.chat_id_, result.id_) then
mod = 'ادمن ،' else mod = '' end
if database:sismember('black:'..bot_id..'vipgp:'..msg.chat_id_, result.id_) then
vip = 'مميز ،' else vip = ''
end
...
database:srem('black:'..bot_id..'dev', result.id_)
database:del('black:'..bot_id..'sudoo'..result.id_..'', 'no')
database:srem('black:'..bot_id..'creatorbasic:'..msg.chat_id_,result.id_)
database:srem('black:'..bot_id..'creator:'..msg.chat_id_, result.id_)
database:srem('black:'..bot_id..'owners:'..msg.chat_id_, result.id_)
database:srem('black:'..bot_id..'mods:'..msg.chat_id_, result.id_)
database:srem('black:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
...
if redis:sismember('black:'..bot_id..'dev',result.sender_user_id_) then
dev = 'المطور ،' else dev = '' end
if database:sismember('black:'..bot_id..'creatorbasic:'..msg.chat_id_, result.sender_user_id_) then
crr = 'منشئ اساسي ،' else crr = '' end
if database:sismember('black:'..bot_id..'creator:'..msg.chat_id_, result.sender_user_id_) then
cr = 'منشئ ،' else cr = '' end
if database:sismember('black:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_) then
own = 'مدير ،' else own = '' end
if database:sismember('black:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_) then
mod = 'ادمن ،' else mod = '' end
if database:sismember('black:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_) then
vip = 'مميز ،' else vip = ''
end
...
if RemBot(msg.sender_user_id_,msg.chat_id_) == 'sudo1' then
database:srem('black:'..bot_id..'dev', result.sender_user_id_)
database:del('black:'..bot_id..'sudoo'..result.sender_user_id_..'', 'no')
database:srem('black:'..bot_id..'creatorbasic:'..msg.chat_id_,result.sender_user_id_)
database:srem('black:'..bot_id..'creator:'..msg.chat_id_, result.sender_user_id_)
database:srem('black:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
database:srem('black:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
database:srem('black:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
elseif RemBot(msg.sender_user_id_,msg.chat_id_) == 'sudo' then
...
------------------------------------------------------------------------
if (text and text == "ابلاغ" and msg.reply_to_message_id_ ~= 0 and (not database:get("black:mute:deleta:msg:"..bot_id..msg.chat_id_))) then 
b = database:get("black:user:deleteL:msg:"..msg.chat_id_..bot_id..msg.sender_user_id_)
if b and b == os.date("%x") then 
send(msg.chat_id_, msg.id_, 1, "👤┇ لا يمكنك الابلاغ اكثر من مره في اليوم", 1, 'html')
else
database:set("black:user:deleteL:msg:"..msg.chat_id_..bot_id..msg.sender_user_id_,os.date("%x"))
...
t = database:get("black:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب '
elseif is_mod(msg) then
t = database:get("black:name_adm"..bot_id..msg.chat_id_) or ' ادمن الكروب خالي '
elseif is_vip(msg) then
t = database:get("black:name_vipp"..bot_id..msg.chat_id_) or 'عضو مميز '
else
t = database:get("black:name_nk"..bot_id..msg.chat_id_) or 'عضو محد يحترمك' 
end
...
if not database:get('black:'..bot_id..'id:mute'..msg.chat_id_) then 
local tahna = (database:get('black:'..bot_id..'nummsg'..msg.chat_id_..result.sender_user_id_) or 0)
if tonumber((database:get('black:'..bot_id..'add:numall'..msg.chat_id_..result.sender_user_id_) or 0)) == 0 then
nko = '0'
else
nko = (database:get('black:'..bot_id..'add:numall'..msg.chat_id_..result.sender_user_id_))
end
local msgs = ((database:get('black:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.sender_user_id_) or 0) + tahna)
local edit = database:get('black:'..bot_id..'user:editmsg'..msg.chat_id_..':'..result.sender_user_id_) or 0
local msg2 = msg
...
if msg.sender_user_id_ == tonumber(sudo_add) then
black_oop = 'المطور الاساسي'
elseif is_sudo(msg2) then
black_oop = database:get("black:name_sudo"..bot_id..msg.chat_id_)  or 'مطور البوت '
elseif is_creatorbasic(msg) then
black_oop = database:get("black:name_cre"..bot_id..msg.chat_id_) or 'منشئ اساسي '
elseif (database:get("black:name_user:"..bot_id..msg.chat_id_..result.id_) and database:get("black:all_sl:"..database:get("black:name_user:"..bot_id..msg.chat_id_..result.id_) ..bot_id..msg.chat_id_)) then 
black_oop = database:get("black:name_user:"..bot_id..msg.chat_id_..result.id_)
elseif is_creator(msg) then
black_oop = database:get("black:name_cre"..bot_id..msg.chat_id_) or 'منشئ الكروب '
elseif (database:get("black:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_) and database:get("black:all_sl:"..database:get("black:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_) ..bot_id..msg.chat_id_)) then 
black_oop = database:get("black:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_)
elseif is_owner(msg2) then
black_oop = database:get("black:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب '
elseif is_mod(msg2) then
black_oop = database:get("black:name_adm"..bot_id..msg.chat_id_) or 'ادمن الكروب'
elseif is_vip(msg2) then
black_oop = database:get("black:name_vipp"..bot_id..msg.chat_id_) or ' عضو مميز '
else
black_oop = database:get("black:name_nk"..bot_id..msg.chat_id_) or 'عضو فقط ' 
end
...
if result.id_ then
if not database:get('black:'..bot_id..'id:mute'..msg.chat_id_) then 
local tahna = (database:get('black:'..bot_id..'nummsg'..msg.chat_id_..result.id_) or 0)
if tonumber((database:get('black:'..bot_id..'add:numall'..msg.chat_id_..result.id_) or 0)) == 0 then
nko = '0'
else
nko = (database:get('black:'..bot_id..'add:numall'..msg.chat_id_..result.id_))
end
local msgs = ((database:get('black:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.id_) or 0) + tahna)
local edit = database:get('black:'..bot_id..'user:editmsg'..msg.chat_id_..':'..result.id_) or 0
local msg2 = msg
msg2.sender_user_id_ = result.id_
local user_msgs = ((database:get('black:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.id_) or 0) + tahna)
local ikeko_text = "غير متفاعل"
if user_msgs then 
if tonumber(user_msgs) < 50 then 
ikeko_text = "غير متفاعل"
...
------------------------------------------------------------------------
if text == ("الرابط") then
if not database:get("black:mute:link:gr:"..bot_id..msg.chat_id_) then 
function dl_cb222( t1,t2 )
if (database:get('black:'..bot_id.."group:link"..msg.chat_id_) and database:get('black:'..bot_id.."group:link"..msg.chat_id_) ~= "Error") then 
send(msg.chat_id_, msg.id_, 1, '🔘┇ *Group Link* \n['..database:get('black:'..bot_id.."group:link"..msg.chat_id_)..']', 1, "md")
...
database:set('black:'..bot_id.."group:link"..msg.chat_id_,link.result)
else 
...
local user_msgs = database:get('black:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
local tahna = (database:get('black:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
if not database:get('black:'..bot_id..'id:mute'..msg.chat_id_) then
...
------------------------------------------------------------------------
if text == ("جهاتي") then
add = (tonumber(database:get('black:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
send(msg.chat_id_, msg.id_, 1, "📨┇عدد اضافه جهاتك ~⪼ *{"..add.."}*\n📨┇سيتم حذف العدد بعد هذه الرساله", 1, 'md')
database:del('black:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
end
...
database:del('black:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_)
local black = '🗑 ┇ تم حذف سحكاتك '
send(msg.chat_id_, msg.id_, 1, black, 1, 'md')
end
------------------------------------------------------------------------
if text == ("اطردني") then
if not database:get('black:'..bot_id..'kickme:mute'..msg.chat_id_) then
redis:set('black:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
redis:set('black:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
...
local yess = redis:get('black:'..bot_id..'kickyess'..msg.sender_user_id_..'')
...
if yess == 'kickyes' then
chat_kick(msg.chat_id_, msg.sender_user_id_)
redis:del('black:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
redis:del('black:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
send(msg.chat_id_, msg.id_, 1, '🕺🏿┇ لقد تم طردك   ', 1, 'md')
end
end
end
if text and text:match("^لا$") then
local noo = redis:get('black:'..bot_id..'kicknoo'..msg.sender_user_id_..'')
if noo == 'kickno' then
redis:del('black:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
redis:del('black:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
...
------------------------------------------------------------------------
if (text and text == (redis:get('black:'..bot_id..'name_bot') or 'تشاكي'))then
name_bot = (redis:get('black:'..bot_id..'name_bot') or 'تشاكي')
local namebot = {
...
local text_sudo = redis:get('black:'..bot_id..'text_sudo')
local nkeko = redis:get('black:'..bot_id..'nmkeko')
local nakeko = redis:get('black:'..bot_id..'nakeko')
if text_sudo then
...
local rules = database:get('black:'..bot_id..'rules'..msg.chat_id_)
if rules then
...
if not database:sismember('black:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_,'ايدي') then
database:sadd('black:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_,'ايدي')
local keko_info = nil
function keko333(extra,result,success)
keko_info = '@'..(result.username_ or 'لا يوجد')..''
local function getpro(extra, result, success)
local all_photo_black = result.total_count_ 
local tahna = (database:get('black:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
if tonumber((database:get('black:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_) or 0)) == 0 then
nko = '0'
else
nko = (database:get('black:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_))
end
local user_msgs = ((database:get('black:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_) or 0) + tahna)
local ikeko_text = "غير متفاعل"
if user_msgs then 
...
t = database:get("black:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_creator(msg) then
t =  database:get("black:name_cre"..bot_id..msg.chat_id_) or 'منشئ الكروب 👨🏻‍✈️'
elseif (database:get("black:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and database:get("black:all_sl:"..database:get("black:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = database:get("black:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_owner(msg) then
t = database:get("black:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب 🏻'   
...
if not database:get('black:'..bot_id..'id:mute'..msg.chat_id_) then
if not database:get('black:'..bot_id..'id:photo'..msg.chat_id_) then
if not database:get("black:gr:id:text:"..bot_id..msg.chat_id_) then 
local keko_text = {
'صورتك فدشي 😘😔❤️  ',
"صارلك شكد مخليه ",
"وفالله 😔💘",
...
local black_new_text = database:get("black:gr:id:text:"..bot_id..msg.chat_id_)
local black_new_text = black_new_text:gsub('#username',(keko_info or 'لا يوجد'))
local black_new_text = black_new_text:gsub('#id',(msg.sender_user_id_ or 'لا يوجد'))
local black_new_text = black_new_text:gsub('#edit',(edit or 'لا يوجد'))
local black_new_text = black_new_text:gsub('#msgs',(user_msgs or 'لا يوجد'))
local black_new_text = black_new_text:gsub('#stast',(t or 'لا يوجد'))
local black_new_text = black_new_text:gsub('#auto',(ikeko_text or 'لا يوجد'))
local black_new_text = black_new_text:gsub('#photos',(all_photo_black or 'لا يوجد'))
local black_new_text = black_new_text:gsub('#game',(nko or 'لا يوجد'))
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_,black_new_text,msg.id_,msg.id_.."")
end
else
if msg.sender_user_id_ == tonumber(sudo_add) then
t = 'المطور الاساسي 👨🏻‍🔧'
...
local black_new_text = black_new_text:gsub('#photos',(all_photo_black or 'لا يوجد'))
local black_new_text = black_new_text:gsub('#game',(nko or 'لا يوجد'))
send(msg.chat_id_, msg.id_, 1, black_new_text, 1, 'html')
end   
end
else
end
...
------------------------------------------------------------------------
if text then
if database:sismember('black:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_,text) then
else
database:del('black:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_) 
end
end
...
send(msg.chat_id_, msg.id_, 1, '📜┇معرف ~⪼ ('..(result.username_..')\n🔘┇الاسم ~⪼ ('..result.first_name_ or "black")..')', 1, 'md')
end
getUser(memb[2],whois)
end
------------------------------------------------------------------------
if text == ("مشاهده منشور") then
database:set('black:'..bot_id..'viewget'..msg.sender_user_id_,true)
...
------------------------------------------------------------------------
if text == 'سمايلات' and database:get('black:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('black:'..bot_id..'l:ids'..msg.chat_id_)
katu = {'🍏','🍎','🍐','🍊','🍋','🍉','🍇','🍓','🍈','🍒','🍑','🍍','🥥','🥝','🍅','🍆','🥑','🥦','🥒','🌶','🌽','🥕','🥔','🥖','🥐','🍞','🥨','🍟','🧀','🥚','🍳','🥞','🥓','🥩','🍗','🍖','🌭','🍔','🍠','🍕','🥪','🥙','🍼','☕️','🍵','🥤','🍶','🍺','🍻','🏀','⚽️','🏈','⚾️','🎾','🏐','🏉','🎱','🏓','🏸','🥅','🎰','🎮','🎳','🎯','🎲','🎻','🎸','🎺','🥁','🎹','🎼','🎧','🎤','🎬','🎨','🎭','🎪','🎟','🎫','🎗','🏵','🎖','🏆','🥌','🛷','🚗','🚙','🚌','🚎','🏎','🚓','🚑','🚚','🚛','🚜','🇮🇶','⚔','🛡','🔮','🌡','💣','📌','📍','📓','📗','📂','📅','📪','📫','📬','📭','⏰','📺','🎚','☎️','📡'}
name = katu[math.random(#katu)]
database:set('black:'..bot_id..'klmos'..msg.chat_id_,name)
taha = '🚀┇اسرع واحد يدز » {`'..name..'`}'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
end
------------------------------------------------------------------------
if text == ''..(database:get('black:'..bot_id..'klmos'..msg.chat_id_) or 'لفاتع')..'' and not database:get('black:'..bot_id..'l:ids'..msg.chat_id_) then
if not database:get('black:'..bot_id..'l:ids'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n🎮| للعب مره اخرى ارسل سمايلات ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('black:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)  
database:incrby('black:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
database:set('black:'..bot_id..'l:ids'..msg.chat_id_,true)
end 
------------------------------------------------------------------------
if text == 'الاسرع' and database:get('black:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('black:'..bot_id..'l:id'..msg.chat_id_)
...
name = katu[math.random(#katu)]
database:set('black:'..bot_id..'klmo'..msg.chat_id_,name)
...
------------------------------------------------------------------------
if text == ''..(database:get('black:'..bot_id..'klmo'..msg.chat_id_) or 'لفاتع')..'' and not database:get('black:'..bot_id..'l:id'..msg.chat_id_) then
if not database:get('black:'..bot_id..'l:id'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n🎮| للعب مره اخرى ارسل الاسرع ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('black:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1) 
database:incrby('black:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
database:set('black:'..bot_id..'l:id'..msg.chat_id_,true)
end 
------------------------------------------------------------------------
if text == 'حزوره' and database:get('black:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('black:'..bot_id..'l:id1'..msg.chat_id_)
...
name = katu[math.random(#katu)]
database:set('black:'..bot_id..'klmoa'..msg.chat_id_,name)
...
------------------------------------------------------------------------
if text == ''..(database:get('black:'..bot_id..'klmoa'..msg.chat_id_) or 'لفاتع')..'' and not database:get('black:'..bot_id..'l:id1'..msg.chat_id_) then
if not database:get('black:'..bot_id..'l:id1'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n🎮| للعب مره اخرى ارسل حزوره ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('black:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)
database:incrby('black:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
database:set('black:'..bot_id..'l:id1'..msg.chat_id_,true)
end 
------------------------------------------------------------------------
if text == 'المعاني' and database:get('black:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('black:'..bot_id..'l:id2'..msg.chat_id_)
...
name = katu[math.random(#katu)]
database:set('black:'..bot_id..'means'..msg.chat_id_,name)
...
------------------------------------------------------------------------
if text == ''..(database:get('black:'..bot_id..'means'..msg.chat_id_) or 'لفاتع')..'' and not database:get('black:'..bot_id..'l:id2'..msg.chat_id_) then
if not database:get('black:'..bot_id..'l:id2'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n🎮| للعب مره اخرى ارسل المعاني ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('black:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)
database:incrby('black:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
database:set('black:'..bot_id..'l:id2'..msg.chat_id_,true)
end 
------------------------------------------------------------------------
if text == 'العكس' and database:get('black:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('black:'..bot_id..'l:id3'..msg.chat_id_)
katu = {'باي','فهمت','موزين','اسمعك','احبك','موحلو','نضيف','حاره','ناصي','جوه','سريع','ونسه','طويل','سمين','ضعيف','شريف','شجاع','رحت','عدل','نشيط','شبعان','موعطشان','خوش ولد','اني','هادئ'}
name = katu[math.random(#katu)]
database:set('black:'..bot_id..'aks'..msg.chat_id_,name)
...
------------------------------------------------------------------------
if text == ''..(database:get('black:'..bot_id..'aks'..msg.chat_id_) or 'لفاتع')..'' and not database:get('black:'..bot_id..'l:id3'..msg.chat_id_) then
if not database:get('black:'..bot_id..'l:id3'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n🎮| للعب مره اخرى ارسل العكس ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('black:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)
database:incrby('black:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
database:set('black:'..bot_id..'l:id3'..msg.chat_id_,true)
end 
------------------------------------------------------------------------
if database:get('black:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_) then  
...
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
return false  end 
local GETNUM = database:get('black:'..bot_id.."GAMES:NUM"..msg.chat_id_)
if tonumber(NUM) == tonumber(GETNUM) then
database:del('black:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sender_user_id_)
database:del('black:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
database:incrby('black:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 5)  
database:incrby('black:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 5)    
taha = '*🔖┇ مبروك فزت ويانه وخمنت الرقم الصحيح\n🚸┇ تم اضافة { 5 } من النقاط *\n'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
elseif tonumber(NUM) ~= tonumber(GETNUM) then
database:incrby('black:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sender_user_id_,1)
if tonumber(database:get('black:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sender_user_id_)) >= 3 then
database:del('black:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sender_user_id_)
database:del('black:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
...
------------------------------------------------------------------------
if database:get('black:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_) then  
if text and text:match("^(%d+)$") then
local NUM = text:match("^(%d+)$")
...
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
return false  end 
local GETNUM = database:get('black:'..bot_id.."GAMES"..msg.chat_id_)
if tonumber(NUM) == tonumber(GETNUM) then
database:del('black:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
taha = '*📮┇ مبروك فزت وطلعت المحيبس بل ايد رقم { '..NUM..' }\n🎊┇ لقد حصلت على { 3 }من نقاط يمكنك استبدالهن برسائل *'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('black:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 3)  
database:incrby('black:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 3)    
elseif tonumber(NUM) ~= tonumber(GETNUM) then
database:del('black:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
...
------------------------------------------------------------------------
if text == 'المختلف' and database:get('black:'..bot_id..'lock_geam'..msg.chat_id_) then
katu = {'😸','☠','🐼','🐇','🌑','🌚','⭐️','✨','⛈','🌥','⛄️','👨‍🔬','👨‍💻','👨‍🔧','👩‍🍳','🧚‍♀','🧜‍♂','🧝‍♂','🙍‍♂','🧖‍♂','👬','👨‍👨‍👧','🕒','🕤','⌛️','📅',
};
name = katu[math.random(#katu)]
database:del('black:'..bot_id..'l:idss'..msg.chat_id_)
database:set('black:'..bot_id..':Set_alii:'..msg.chat_id_,name)
...
------------------------------------------------------------------------
if text == 'امثله' and database:get('black:'..bot_id..'lock_geam'..msg.chat_id_) then
katu = {
'جوز','ضراطه','الحبل','الحافي','شقره','بيدك','سلايه','النخله','الخيل','حداد','المبلل','يركص','قرد','العنب','العمه','الخبز','بالحصاد','شهر','شكه','يكحله',
};
name = katu[math.random(#katu)]
database:set('black:'..bot_id..':Set_Amthlh:'..msg.chat_id_,name)
database:del('black:'..bot_id..'l:idst'..msg.chat_id_)
...
------------------------------------------------------------------------
if text == ''..(database:get('black:'..bot_id..':Set_alii:'..msg.chat_id_) or '57999')..'' then -- // المختلف
if not database:get('black:'..bot_id..'l:idss'..msg.chat_id_) then 
database:incrby('black:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)  
database:incrby('black:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
database:del('black:'..bot_id..':Set_alii:'..msg.chat_id_)
taha = '*  🎁  احسنت اجابتك صحيحه   \n* '
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
end
database:set('black:'..bot_id..'l:idss'..msg.chat_id_,true)
end
------------------------------------------------------------------------
if text == ''..(database:get('black:'..bot_id..':Set_Amthlh:'..msg.chat_id_) or '6499')..'' then -- // امثله
if not database:get('black:'..bot_id..'l:idst'..msg.chat_id_) then 
database:incrby('black:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)  
database:incrby('black:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
database:del('black:'..bot_id..':Set_Amthlh:'..msg.chat_id_)
taha = '*  🎁  احسنت اجابتك صحيحه   \n* '
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
end
database:set('black:'..bot_id..'l:idst'..msg.chat_id_,true)
end
------------------------------------------------------------------------
if text == 'محيبس' or text == 'بات' then
if database:get('black:'..bot_id..'lock_geam'..msg.chat_id_) then   
Num = math.random(1,6)
database:set('black:'..bot_id.."GAMES"..msg.chat_id_,Num) 
TEST = [[
*➀       ➁     ➂      ➃      ➄     ➅
↓      ↓     ↓      ↓     ↓     ↓
👊 ‹› 👊 ‹› 👊 ‹› 👊 ‹› 👊 ‹› 👊
...
send(msg.chat_id_, msg.id_, 1,TEST, 1, 'md')
database:setex('black:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 100, true)  
return false  
end
end
------------------------------------------------------------------------
...
database:set('black:'..bot_id.."GAMES:NUM"..msg.chat_id_,Num) 
TEST = '*\n🎮┇ اهلا بك عزيزي في لعبة التخمين :\nٴ━━━━━━━━━━\n'..'⚠┇ ملاحظه لديك { 3 } محاولات فقط فكر قبل ارسال تخمينك \n\n'..'🔖┇ سيتم تخمين عدد ما بين ال {1 و 20} اذا تعتقد انك تستطيع الفوز جرب واللعب الان ؟ \n💥*'
send(msg.chat_id_, msg.id_, 1,TEST, 1, 'md')
database:setex('black:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 100, true)  
return false  
end
end
------------------------------------------------------------------------
if text =='مجوهراتي' then 
if tonumber((database:get('black:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_) or 0)) == 0 then
taha = '*💎┇ ليس لديك مجوهرات \n📬┇ للحصول على مجوهرات ارسل الاسرع وابدأ اللعب*\n'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
else
taha = '*💎┇ عدد مجوهراتك الحاليه  ('..(database:get('black:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_))..')*'
taha1 = '*💎┇ مجموع مجوهراتك  ('..(database:get('black:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_))..')*'
...
taha = (database:get('black:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_) * tonumber(database:get('black:'..bot_id..'gamepoint' .. msg.chat_id_)or 50))
database:incrby('black:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_,taha)  
database:del('black:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_)
taha = tonumber((database:get('black:'..bot_id..'gamepoint' .. msg.chat_id_) or 50))
...
database:del('black:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_)  
database:del('black:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_)  
send(msg.chat_id_, msg.id_, 1, "تم حذف جميع مجوهراتك", 1, "md") 
end
------------------------------------------------------------------------
if text == 'حذف رسائلي' then
database:del('black:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_)
database:del('black:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, "🗑 ┇ تم حذف رسائلك  ", 1, "md") 
end
------------------------------------------------------------------------
if text == ("الاوامر") and (is_mod(msg) or is_creatorbasic(msg)) then
local help = redis:get('black:'..bot_id..'help')
local text =  [[
📮┇هناك {4} اوامر لعرضها
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
...
local h1 = redis:get('black:'..bot_id..'h1')
local text =  [[
...
local h2 = redis:get('black:'..bot_id..'h2')
local text =  [[
🥈┇اوامر الادمنيه
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
🚫┇كتم
...
local h3 = redis:get('black:'..bot_id..'h3')
local text =  [[
🥇┇ اوامر المدراء
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
...
local hres = redis:get('black:'..bot_id..'hres')
local text =[[
...
local h4 = redis:get('black:'..bot_id..'h4')
local text =  [[
🎖┇اوامر المطور
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
✳️┇تفعيل/تعطيل
...
📜┇[قناه شروحات سورس](http://t.me/blackTEAM)

📥┇[قناه ملفات السورس](t.me/blackTEAM)
...
database:del( 'black:'..bot_id.."charge:"..'-100'..data.channel_.id_)
database:srem("thsake:gog"..bot_id, '-100'..data.channel_.id_)
database:sadd( 'black:'..bot_id.."groups",'-100'..data.channel_.id_)
end
end
local chat = {}
if (data.ID == "UpdateNewMessage") then
local Data_black = data
...
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data) 
if data.username_ ~= false then
database:hset('black:'..bot_id..'user:'..msg.sender_user_id_,'username',(data.username_ or msg.sender_user_id_))
...
local Bots = database:get("lock_bot:black"..msg.chat_id_..bot_id)
for i=0,#mem_id do  
...
if msg.content_.ID == "MessageChatAddMembers" then  
local mem_id = msg.content_.members_  
local Bots = database:get("lock_botAndBan:black"..msg.chat_id_..bot_id) 
for i=0,#mem_id do  
if msg.content_.members_[i].type_.ID == "UserTypeBot" and not is_mod(msg) and Bots then   
...
if database:get("black:edit:text:su:newamr2:"..bot_id..data.message_.chat_id_..data.message_.content_.text_) then
local black_edit_text = database:get("black:edit:text:su:newamr2:"..bot_id..data.message_.chat_id_..data.message_.content_.text_)
print(black_edit_text)
if black_edit_text then
data.message_.content_.text_ = (black_edit_text or data.message_.content_.text_)
end
end
end
local Name_Bot = (redis:get('black:'..bot_id..'name_bot') or 'تشاكي')
...
local msgg = result 
database:incr('black:'..bot_id..'user:editmsg'..msgg.chat_id_..':'..msgg.sender_user_id_)
...
if database:get("lock_edit:black"..msg.chat_id_..bot_id) and not result.content_.text_ and not is_creator(msgg) and not is_creatorbasic(msgg) then
...
send(msg.chat_id_,0, 1, "[⚠┇#تحذير] \n🔖┇قام شخصآ ما في المجموعه بالتعديل على الميديا يرجى الانتباه \n📫┇الشخص الي قام بالتعديل \n ["..data.first_name_.."](T.ME/blackTEAM)" , 1, 'md') 
end
end
getUser(result.sender_user_id_,get_edit)
delete_msg(msg.chat_id_,{[0] = msg.message_id_}) 
elseif database:get("lock_edit:black:kick"..msg.chat_id_..bot_id) and not result.content_.text_ and not is_creator(msgg) and not is_creatorbasic(msgg) then
function get_edit(arg,data)
local username = data.username_
local name = data.first_name_
local iduser = data.id_
...
send(msg.chat_id_,0, 1, "[⚠┇#تحذير] \n🔖┇قام شخصآ ما في المجموعه بالتعديل على الميديا يرجى الانتباه \n📫┇الشخص الي قام بالتعديل \n ["..data.first_name_.."](T.ME/blackTEAM)\n🚷┇ وتم طرده \n" , 1, 'md') 
end
...
------------------------------------------------------------------------
local taha = database:get('black:'..bot_id.."add:reply:rd"..text..msg.chat_id_)   
...
------------------------------------------------------------------------
--         »»                 End UpdateChat                          ««              --
elseif (data.ID == "UpdateOption" and data.name_ == "my_id") then  local list = database:smembers('black:'..bot_id.."userss") for k,v in pairs(list) do tdcli_function({ID='GetChat',chat_id_ = v},function(arg,data) end,nil) end 
local list = database:smembers('black:'..bot_id..'groups') 
for k,v in pairs(list) do 
...
database:srem('black:'..bot_id..'pro:groups',v) 
database:srem( 'black:'..bot_id.."groups",v) 
end
if data and data.type_ and data.type_.channel_ and data.type_.channel_.status_ and data.type_.channel_.status_.ID == "ChatMemberStatusLeft" then
database:srem("thsake:gog"..bot_id,v) 
database:srem('black:'..bot_id..'pro:groups',v) 
...
database:sadd( 'black:'..bot_id.."groups",v) 
...
local msg = data.message_
local text = msg.content_.text_
local Get_Msg_Pin = database:get('black:'..bot_id..'Msg_Text'..msg.chat_id_)
if Get_Msg_Pin ~= nil then
if text == Get_Msg_Pin then
tdcli_function ({ID = "PinChannelMessage",channel_id_ = msg.chat_id_:gsub('-100',''),message_id_ = msg.id_,disable_notification_ = 0},function(arg,d) if d.ID == 'Ok' then;database:del('black:'..bot_id..'Msg_Text'..msg.chat_id_);end;end,nil)   
elseif (msg.content_.sticker_) then 
print(msg.content_.sticker_.sticker_.persistent_id_)
if Get_Msg_Pin == msg.content_.sticker_.sticker_.persistent_id_ then
tdcli_function ({ID = "PinChannelMessage",channel_id_ = msg.chat_id_:gsub('-100',''),message_id_ = msg.id_,disable_notification_ = 0},function(arg,d) database:del('black:'..bot_id..'Msg_Text'..msg.chat_id_) end,nil)   
end
end
if (msg.content_.voice_) then
